                    ...
                    <tr>
                      ...
                      <td colspan="3" style="border-right: 1px solid #cdd0d4;border-right: 2px solid #cdd0d4;">
                        ...
                        <?php if(!empty($reports['month_year_arr_pqoh'][$current_year][str_pad($i,2,"0",STR_PAD_LEFT)]['total_pqoh'])){ ?>
                            &nbsp;
                            <?php $pqohvalue= (int)$reports['month_year_arr_pqoh'][$current_year][str_pad($i,2,"0",STR_PAD_LEFT)]['total_pqoh']; $adjvaluereset += $pqohvalue; ?>
                        <?php } else { ?>
                            &nbsp;
                        <?php } ?>
                        <!---->
                        ...
                        <?php if(!empty($reports['month_year_arr_cqoh'][$current_year][str_pad($i,2,"0",STR_PAD_LEFT)]['total_cqoh'])){ ?>
                            &nbsp;
                        <?php $cqohvalue= (int)$reports['month_year_arr_cqoh'][$current_year][str_pad($i,2,"0",STR_PAD_LEFT)]['total_cqoh']; $adjvaluereset += $cqohvalue; ?>
                        <?php } else { ?>
                            &nbsp;
                        <?php } ?>
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <select class="form-control" name="search_by_item">
                  ...
                  <option value="openingqoh">Opening QoH</option>
...
<script type="text/javascript">
  $(document).on('click', '.item_movement_btn', function(event) {
    ...
    if(data_by == 'receive')
        {
            $('#item_movement_by_date_selection_table > thead > tr').html('<th>Action</th><th>Date</th><th class="text-right">Qty</th><th class="text-right">Pack Qty</th><th class="text-right">Size</th><th class="text-right">Price</th><th class="text-right">PO Number</th>');
        }
    else if(data_by == 'sold')
        {
            $('#item_movement_by_date_selection_table > thead > tr').html('<th id="first_th">Print Receipt</th><th>Action</th><th>Date</th><th class="text-right">Qty</th><th class="text-right">Pack Qty</th><th class="text-right">Size</th><th class="text-right">Price</th>');
        }
    ...
    $.getJSON(item_movement_data_url, function(result){
      ...
      if(result.length){
        ...
        $.each(result,function(i, v) {
          ...
          html += '</td>';
          if(data_by != "adjustment" && data_by != "qohupdate" && data_by!="invreset" && data_by!="openingqoh" && data_by!="perentchild" && data_by!="childparent" && data_by!="phoneadjustment" && data_by!="openingqohphone")
          {
          html += '<td class="text-right">';
          ...
          total_amount += parseFloat(v.total_price);
          html += '</td>';
          }
          if(data_by == 'receive')
          {
        ...
        if(data_by == 'sold')
        {
            html += '<tr><th colspan="3" class="text-right">Total</th><th class="text-right">'+total_qty+'</th><th></th><th></th><th class="text-right">'+parseFloat(total_amount).toFixed(4)+'</th></tr>';
        }
        else if(data_by == 'receive')
        {
            html += '<tr><th colspan="2" class="text-right">Total</th><th class="text-right">'+total_qty+'</th><th></th><th></th><th class="text-right">'+parseFloat(total_amount).toFixed(4)+'</th><th></th></tr>';
        }
        else if(data_by == 'adjustment'){
            html += '<tr><th colspan="2" class="text-right">Total</th><th class="text-right">'+total_qty+'</th><th></th></tr>';
        }
        console.log(html);
