...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on('keyup', '.buydown_amt', function(event){
            ...
            var cost = parseFloat($(this).parents('tr').find('td.live_cost').html());
            
            var costAfter = cost - currAmt;
        ...
        $(document).on('keyup', '#buydown_amount', function(event){
            ...
            if(($('#item_listing2 tbody tr').length) > 0) {
                ...
                $('#item_listing2 tbody tr').each(function(){
                    ...
                    cost = parseFloat($(this).find('td.live_cost').html());

                    costAfter = cost - buyDownAmt;
...
// console.log(existingBuydownItems);
    ...
    $(document).on('click', '#submit_budown', function(e){
        ...
        if(added_items.length > 0){
            <?php if(session()->get('hq_sid') == 1){ ?>
                var buydown_id = '<?php echo $buydown_info->buydown_id; ?>';
                $.ajax({
                        url: "<?php echo url('/editbuydownhqcheck'); ?>",
                        method: 'post',
                        headers: {
                              'X-CSRF-TOKEN': '<?php echo csrf_token();  ?>'
                        },
                        data: JSON.stringify({buy_down: buydown_id}),
                        type : 'POST',
