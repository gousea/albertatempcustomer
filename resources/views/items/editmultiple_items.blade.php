  ...
  $(document).on('change', '.iitemid', function(event) {
    ...
    var itemIds = keys.filter(function(key){
      ...
      $.ajax({
        ...
        data : {session_items_total_ids:session_items_total_ids, items_total_ids:items_total_ids, main_arr:main_arr},
        type : 'POST',
        dataType: 'json',
        success: function(data) {
            console.log(1179);
...
<script type="text/javascript">
  $(document).on('click', '#update_item_btn', function(event) {
    ...
    $.ajax({
            ...
            data: {session_items_total_ids:session_items_total_ids, items_total_ids:items_total_ids},
  ...
  $(document).on('click', '#item_update_sure_btn', function(event) {
    ...
    <?php if(session()->get('hq_sid') == 1){ ?> 
        ...
        var div =   '<center><button class="btn btn-danger buttonload"><i class="fa fa-circle-o-notch fa-spin"></i>Loading</button></center>';
        $('#loader').html(div);
        $.ajax({
