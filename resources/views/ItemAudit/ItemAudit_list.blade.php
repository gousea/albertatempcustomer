...
@section('main-content')
    ...
    <div id="content">
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-12">
                    <form action="{{ route('ItemAuditListForm') }}" method="GET" id="form_item_search"
                        class="form-inline row">
                        <div class="col-md-2" style="margin-right: 70px;">
                            <input type='text' class="form-control adjustment-fields" name="dates"
                                value="<?php echo isset($start_date) ? $start_date : ''; ?>" id="dates" placeholder="Select Date Range" autocomplete="off"
                                style="width: 220px;" readonly />
                            <input type='hidden' class="form-control" name="start_date" value="<?php echo isset($start_date) ? $start_date : ''; ?>"
                                id="start_date" placeholder="Start Date" readonly />
                            <input type="hidden" class="form-control" name="end_date" value="<?php echo isset($end_date) ? $end_date : ''; ?>"
                        ...
                        <div class="col-md-2">
                            <select name="userid" class="sample-class adjustment-fields" id="user_code" placeholder="Users">
                                ...
                                <option value="All" <?php if (isset($_GET['userid']) && $_GET['userid'] == 'All') {
    echo 'selected="selected"';
                        ...
                        <div class="col-md-2">
                            <select name="itemname" class="sample-class1 adjustment-fields" id="itemname"
                                placeholder="items">
                                <option value="All">Select Items</option>
                                <option value="All" <?php if (isset($_GET['itemname']) && $_GET['itemname'] == 'All') {
    echo 'selected="selected"';
} ?>>All</option>
                        ...
                        <div class="col-md-2">
                            <select name="mtype" class="sample-class1 adjustment-fields" id="mtype"
                                placeholder="modification" required>
                                <option value="">Select modification</option>
                                <option value="Item Type" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Item Type') {
    echo 'selected="selected"';
} ?>>Item Type</option>
                                <option value="Item Name" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Item Name') {
    echo 'selected="selected"';
} ?>>Item Name</option>
                                <option value="Unit" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Unit') {
    echo 'selected="selected"';
} ?>>Unit</option>
                                <option value="Cost" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Cost') {
    echo 'selected="selected"';
} ?>>Cost</option>
                                <option value="Price" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Price') {
    echo 'selected="selected"';
} ?>>Price</option>
                                <option value="Department" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Department') {
    echo 'selected="selected"';
} ?>>Department</option>
                                <option value="Category" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Category') {
    echo 'selected="selected"';
} ?>>Category</option>
                                <!-- <option value="Sub Category">Sub Category</option> not complited-->
                                <option value="Selling Unit" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Selling Unit') {
    echo 'selected="selected"';
} ?>>Selling Unit</option>
                                <option value="Size" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Size') {
    echo 'selected="selected"';
} ?>>Size</option>
                                <option value="Unit Per Case" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Unit Per Case') {
    echo 'selected="selected"';
} ?>>Unit Per Case</option>
                                <option value="Supplier" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Supplier') {
    echo 'selected="selected"';
} ?>>Supplier</option>
                                <option value="Qty on Hand" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Qty on Hand') {
    echo 'selected="selected"';
} ?>>Qty on Hand</option>
                                <option value="Bottle Deposit" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Bottle Deposit') {
    echo 'selected="selected"';
} ?>>Bottle Deposit</option>
                                <option value="Food Item" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Food Item') {
    echo 'selected="selected"';
} ?>>Food Item</option>
                                <!--<option value="Promotion">Promotion</option> not complited-->
                                <option value="Age Verification" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Age Verification') {
    echo 'selected="selected"';
} ?>>Age Verification</option>
                                <option value="WIC" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'WIC') {
    echo 'selected="selected"';
} ?>>WIC</option>
                                <option value="Status" <?php if (isset($_GET['mtype']) && $_GET['mtype'] == 'Status') {
    echo 'selected="selected"';
} ?>>Status</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <input type="submit" name="search_filter" value="Generate"
                                class="btn btn-success rcorner header-color basic-button-small" style="margin-right:0px">
                        </div>
                    </form>
                </div>
                ...
                <div class="row col-md-12">
                    <div class="table-responsive">
                        <table id="vendor" class="table table-hover promotionview dataTable no-footer" style="width: 100%;"
                            role="grid">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-left">
                                        ...
                                        <a id="view_cat0"
                                            onclick="viewfunc('<?= $v['beforem'] ?>', '<?= $v['beforem'] ?>');"
                                            data-toggle="tooltip" title="View"
                                            class="btn small-btn"><?php echo $v['beforem']; ?></a>
                                        <?php }
                                                elseif($mtype=='Category'){?>
                                        <a id="view_cat1" onclick="viewcat('<?= $v['beforem'] ?>', '<?= $v['beforem'] ?>');"
                                            data-toggle="tooltip" title="View"
                                            class="btn small-btn"><?php echo $v['beforem']; ?></a>
                                        <?php }
                                    ...
                                    <td class="text-left">
                                        <?php if($mtype=='Department'){?>
                                        <a id="view_cat3" onclick="viewfunc('<?= $v['afterm'] ?>', '<?= $v['afterm'] ?>');"
                                            data-toggle="tooltip" title="View"
                                            class="btn small-btn"><?php echo $v['afterm']; ?></a>
                                        <?php }
                                                elseif($mtype=='Category'){?>
                                        <a id="view_cat4" onclick="viewcat('<?= $v['afterm'] ?>', '<?= $v['afterm'] ?>');"
                                            data-toggle="tooltip" title="View"
                                            class="btn small-btn"><?php echo $v['afterm']; ?></a>
                                        <?php }
                                    ...
                                    <td class="text-left">
                                        <a id="view_userid"
                                            onclick="view_userid('<?= $v['userid'] ?>', '<?= $v['userid'] ?>');"
                                            data-toggle="tooltip" title="View"
                                            class="btn small-btn"><?php echo $v['userid']; ?></a>
                                        <span><?php //echo $v['userid'];
                                        ?></span>
                                    </td>
                                </tr>
                                <?php } ?>
...
<script src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js"></script> --}}
    ...
    <script>
        $(document).ready(function() {
            ...
            function cb(start, end) {
                $('input[name="dates"]').html(start.format('MMMM D, YYYY hh:mm') + '-' + end.format(
                    'MMMM D, YYYY hh:mm '));
    ...
    <style>
        ...
        #view_userid, #view_cat0, #view_cat3{
            color: blue;
        ...
        #view_userid:hover, #view_cat0:hover, #view_cat3:hover{
            color: #286fb7;
            text-decoration: underline;
            font-weight: bolder;
            font-size: 15px;
