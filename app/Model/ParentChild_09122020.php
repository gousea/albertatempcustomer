            ...
            foreach($stores as $store){
                ...
                try {
                      ...
                      $ipiid = $query1['ipiid'];
                        ...
                        DB::connection('mysql')->select("INSERT INTO u".$store.".trn_physicalinventorydetail SET  ipiid = '" . (int)$ipiid . "',
                          ...
                          foreach($child_items as $chi_item){
                              ...
                              if(count($trn_webadmin_history)){
                                ...
                                unset($old_item_values['itemimage']);
                                  ...
                                  DB::connection('mysql')->select("INSERT INTO u".$store.".trn_webadmin_history SET  itemid = '" . ($chi_item['iitemid']) . "',userid = '" . Auth::user()->id . "',barcode = '" . ($chi_item['vbarcode']) . "', type = 'Cost', oldamount = '" . $chi_item['dcostprice'] . "', newamount = '". (($chi_item['npack']) * (($chi_item['nunitcost']))) ."', source = 'ItemEditAddParent', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
                                  }
                                  catch (QueryException $e) {
                                      Log::error($e);
                                  }
                      ...
                      DB::connection('mysql')->select("UPDATE u".$store.".mst_item SET  iqtyonhand = '". (int)($quatity_on_hand) ."', npack= '". (int)($data['cnpack']) ."',nsellunit='". (int)($data['cnpack']) ."'  WHERE iitemid= '". (int)($parent_item['iitemid']) ."'");
                      ...
                      if(count($trn_webadmin_history)){
                        ...
                        DB::connection('mysql')->select("UPDATE u".$store.".trn_webadmin_history SET general = '" . $x_general . "' WHERE historyid = '" . (int)$trn_webadmin_history_last_id . "'");
                        }
                        catch (QueryException $e) {
                            Log::error($e);
                        }
        ...
        if(isset($data['stores'])){
            ...
            foreach($data['stores'] as $store){
                try {
                    ...
                    $childitem = DB::connection("mysql")->select("select iitemid from u".$store.".mst_item where vbarcode = '".$childbarcode[0]->vbarcode."' ");
                    $parentitem = DB::connection("mysql")->select("select iitemid from u".$store.".mst_item where vbarcode = '".$parentbarcode[0]->vbarcode."' ");
                    
                    
                    foreach($childitem as $item){
                        $child_id= $item->iitemid;
                    }
                    foreach($parentitem as $item){
                        $parent_id= $item->iitemid;
                    }
                    ...
                    DB::connection('mysql')->update("UPDATE u".$store.".mst_item SET isparentchild=0,parentid=0,parentmasterid=0 WHERE iitemid= '". (int)($child_id) ."'");
                    ...
                    if($remove_parent_item_update['isparentchild'] == 2){
                        DB::connection('mysql')->update("UPDATE u".$store.".mst_item SET isparentchild=0,parentid=0,parentmasterid=0 WHERE iitemid= '". (int)($parent_id) ."'");
                    }
                }
