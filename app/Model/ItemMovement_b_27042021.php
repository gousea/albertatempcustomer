...
class ItemMovement extends Model
{
    ...
    public function getItemMovementReport($data) {
	    ...
	    $query = "SELECT a.iitemid, a.vitemtype, a.vbarcode, a.vitemname, a.npack, a.IQTYONHAND as qoh ,CASE WHEN a.NPACK = 1 or (a.npack is null)   then a.IQTYONHAND else (Concat(cast(((a.IQTYONHAND div a.NPACK )) as signed), '  (', Mod(a.IQTYONHAND,a.NPACK) ,')') ) end as IQTYONHAND FROM mst_item as a WHERE a.vbarcode='". ($data['search_vbarcode']) ."'";
...
// 		 dd($sql_ajustment);                   
       ...
       // dd($pqoh_data);
        ...
        //hgb OPNING QOH of pos 
        
        $sql_posqoh ="SELECT ifnull(SUM(tpd.ndebitqty),0) as total_posqoh, DATE_FORMAT(tp.LastUpdate,'%m-%Y') as dcreatedate 
        FROM trn_physicalinventorydetail tpd left join trn_physicalinventory tp on tp.ipiid = tpd.ipiid
        where vitemid='". ($data['search_iitemid']) ."' 
        And  vtype='Opening QOH POS'
        AND (date_format(tp.LastUpdate,'%Y') BETWEEN '".$start_date."' AND '".$end_date."') GROUP BY date_format(tp.LastUpdate,'%m-%Y')";
        $posqoh_data = DB::connection('mysql_dynamic')->select($sql_posqoh);
		...
		//POS START 
		$year_arr_posqoh = array();
		$month_year_arr_posqoh = array();
		
		$posqoh_data = array_map(function ($value) {
            return (array)$value;
        }, $posqoh_data);
		if(count($posqoh_data) > 0){
            
			foreach ($posqoh_data as $key => $value) {
				$t = explode('-', $value['dcreatedate']);

				if(array_key_exists($t[1], $year_arr_posqoh)){
                    $year_arr_posqoh[$t[1]]['total_posqoh'] = $year_arr_posqoh[$t[1]]['total_posqoh'];
				}else{
                    $year_arr_posqoh[$t[1]]['total_posqoh'] = $value['total_posqoh'];
				}

				if(array_key_exists($t[1], $month_year_arr_posqoh)){
					if(array_key_exists($t[0], $month_year_arr_posqoh[$t[1]])){
                        $month_year_arr_posqoh[$t[1]][$t[0]]['total_posqoh'] = $month_year_arr_posqoh[$t[1]][$t[0]]['total_posqoh'];
					}else{
                        $month_year_arr_posqoh[$t[1]][$t[0]]['total_posqoh'] = $value['total_posqoh'];
					}
				}else{
                    $month_year_arr_posqoh[$t[1]][$t[0]]['total_posqoh'] = $value['total_posqoh'];
				}
	...
	//	dd($return['month_year_arr_adjustment_phy'] );
		...
		$return['year_arr_posqoh'] = $year_arr_posqoh;
		$return['month_year_arr_posqoh'] = $month_year_arr_posqoh;
 ...
 //dd($return);
	...
	public function getItemMovementData($vbarcode,$start_date,$end_date,$data_by) {
        ...
        switch ($data_by) {
            ...
            case "sold":
                ...
                date_format(trn_s.dtrandate,'%m-%d-%Y %h:%i %p') as ddate, trn_sd.npack as npack, trn_sd.vsize as size,
                trn_sd.nunitprice as dunitprice, trn_sd.ndebitqty as total_qoh, trn_sd.nextunitprice as total_price ,preqoh as before_sold_qoh,(preqoh-ndebitqty) as after_sold_qoh
                FROM trn_salesdetail trn_sd , trn_sales trn_s WHERE trn_s.vtrntype='Transaction' AND trn_s.isalesid=trn_sd.isalesid 
                AND trn_sd.vitemcode='". ($vbarcode) ."' AND date_format(trn_s.dtrandate,'%Y-%m-%d') >= '".$start_date."' AND date_format(trn_s.dtrandate,'%Y-%m-%d') <= '".$end_date."' ORDER BY ddate ASC";
                $query_data = DB::connection('mysql_dynamic')->select($sql);
            ...
            case "receive":
                ...
                DATE_FORMAT(tr.dcreatedate,'%m-%d-%Y %h:%i %p') as ddate, ifnull(trd.before_rece_qoh,'') as before_rece_qoh, ifnull(trd.after_rece_qoh,'') as after_rece_qoh
                FROM trn_receivingorderdetail trd LEFT JOIN trn_receivingorder tr 
                ON (trd.iroid = tr.iroid) WHERE trd.vbarcode='". ($vbarcode) ."' 
                AND tr.vordertype='PO'
                AND tr.estatus='Close'
                AND date_format(tr.dcreatedate,'%Y-%m-%d') >= '".$start_date."' AND date_format(tr.dcreatedate,'%Y-%m-%d') <= '".$end_date."' ORDER BY ddate ASC";
            ...
            case "openingqoh":
                ...
                AND date_format(tp.LastUpdate,'%Y-%m-%d') <= '".$end_date."' ORDER BY ddate ASC ";
                $query_data = DB::connection('mysql_dynamic')->select($sql1);
                break;
            ...
            case "adjustment":
                ...
                $sql1 = "SELECT vtype,ifnull(tp.vrefnumber,0) as vrefnumber,tpd.ndebitqty as items_count, DATE_FORMAT(tp.LastUpdate,'%m-%d-%Y %h:%i %p') as ddate, tpd.beforeQOH,  tpd.afterQOH
                FROM trn_physicalinventorydetail tpd LEFT JOIN trn_physicalinventory tp ON (tpd.ipiid = tp.ipiid) 
                and  vtype in('Quick Update','Inventory Reset','Child Update','Adjustment','Physical','Conv Case','Perent Update')  WHERE tpd.vitemid='". ($itemid) ."' AND date_format(tp.LastUpdate,'%Y-%m-%d') >= '".$start_date."' 
                AND date_format(tp.LastUpdate,'%Y-%m-%d') <= '".$end_date."' ORDER BY ddate ASC";
            ...
            case "phoneadjustment":
                ...
                $sql1 = "SELECT ifnull(tp.vrefnumber,0) as vrefnumber,tpd.ndebitqty as items_count, DATE_FORMAT(tp.LastUpdate,'%m-%d-%Y %h:%i %p') as ddate, tpd.beforeQOH,  tpd.afterQOH
                FROM trn_physicalinventorydetail tpd LEFT JOIN trn_physicalinventory tp ON (tpd.ipiid = tp.ipiid) 
            ...
            case "openingqohphone":
                ...
                $sql1 = "SELECT ifnull(tp.vrefnumber,0) as vrefnumber,tpd.ndebitqty as items_count, DATE_FORMAT(tp.LastUpdate,'%m-%d-%Y %h:%i %p') as ddate,
                tpd.beforeQOH, tpd.afterQOH FROM trn_physicalinventorydetail tpd LEFT JOIN trn_physicalinventory tp ON (tpd.ipiid = tp.ipiid) 
                And  vtype='Opening QOH POS'  WHERE tpd.vitemid='". ($itemid) ."' AND date_format(tp.LastUpdate,'%Y-%m-%d') >= '".$start_date."' 
                AND date_format(tp.LastUpdate,'%Y-%m-%d') <= '".$end_date."' ORDER BY ddate ASC";
    ...
    //new parent and child data
     public function getItemMovementReport_child($data_list) {
        
        $list=array();
       foreach($data_list as $v){
            $data['search_iitemid']=$v->iitemid;
            $data['search_vbarcode']=$v->vbarcode;
...
// 		 dd($sql_ajustment);                   
       ...
       // dd($pqoh_data);
        ...
        $sql_cqoh ="SELECT ifnull(SUM(tpd.ndebitqty),0) as total_cqoh, DATE_FORMAT(tp.LastUpdate,'%m-%Y') as dcreatedate FROM trn_physicalinventorydetail tpd 
                    left join trn_physicalinventory tp on tp.ipiid = tpd.ipiid where vitemid='". ($data['search_iitemid']) ."' 
                    And  vtype='Perent Update' AND tp.estatus='Close' AND (date_format(tp.dcreatedate,'%Y') BETWEEN '".$start_date."' AND '".$end_date."') 
                    GROUP BY date_format(tp.LastUpdate,'%m-%Y')";
            $cqoh_data = DB::connection('mysql_dynamic')->select($sql_cqoh);
        //perent Qoh update end 
