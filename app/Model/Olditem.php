            ...
            foreach ($datas as $key => $data) {
                ...
                if(isset($stores_hq)){
                    ...
                    foreach($stores as $store){
                        ...
                        try {
                            DB::connection('mysql')->update("UPDATE u".$store.".mst_itempackdetail SET  npackprice = '" . ($data['npackprice']) . "', npackmargin = '" . ($data['npackmargin']) . "' WHERE iitemid = '".(int)($iitem_id)."' AND idetid= '".(int)($idet_pack_id)."' ");
                        }
                        catch (QueryException $e) {
                            $error['error'] = $e->getMessage(); 
                            return $error; 
