                ...
                try {
                    if(count($data['item_ids']) > 0){
                        foreach($data['item_ids'] as $value){
                                ...
                                foreach($barcode as $skucode){
                                    $vbarcode = $skucode->vbarcode;
                                }
                                
                                foreach($stores as $store){
                                        $updated_column = array();
                                        $current_item = DB::connection('mysql')->select("SELECT * FROM u".$store.".mst_item WHERE vbarcode='".$vbarcode."'");
                                        ...
                                        if($current_item['dcostprice'] != $new_dcostprice){
                                            DB::connection('mysql')->select("INSERT INTO u".$store.".trn_itempricecosthistory SET  iitemid = '" . $current_item['iitemid'] . "',vbarcode = '" . ($current_item['vbarcode']) . "', vtype = 'EitemCost', noldamt = '" . ($current_item['dcostprice']) . "', nnewamt = '" . ($new_dcostprice) . "', iuserid = '" . Auth::user()->id . "', dhistorydate = CURDATE(), thistorytime = CURTIME(),SID = '" . (int)(session()->get('sid'))."'");
                                        }
                                        ...
                                        if(isset($data['update_iitemgroupid']) && $data['update_iitemgroupid'] != 'no-update'){
                                            ...
                                            DB::connection('mysql')->select("DELETE FROM u".$store."itemgroupdetail WHERE vsku='" . ($isParentCheck['vbarcode']) . "'");
                                        ...
                                        if(($isParentCheck['vitemtype']) == 'Lot Matrix'){
                                            ...
                                            if(count($itempackexist) > 0){
                                                DB::connection('mysql')->select("UPDATE u".$store.".mst_itempackdetail SET `ipack` = '" . (int)$ipack . "',`npackcost` = '" . $npackcost . "',`nunitcost` = '" . $nunitcost . "',`npackprice` = '" . $npackprice . "',`npackmargin` = '" . $npackmargin . "' WHERE vbarcode='". ($isParentCheck['vbarcode']) ."' AND iitemid='". (int)$current_item['iitemid'] ."' AND iparentid=1");
                                            }
                                        }
                                    }
                                }
