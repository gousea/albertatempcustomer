            ...
            foreach($data as $value){
                try {
                    ...
                    if(count($exist_ipodetid) > 0){
                        DB::connection('mysql_dynamic')->select("INSERT INTO mst_delete_table SET  TableName = 'trn_purchaseorderdetail',`Action` = 'delete',`TableId` = '" . (int)$value . "',SID = '" . (int)(session()->get('sid'))."'");
                        DB::connection('mysql_dynamic')->select("DELETE FROM trn_purchaseorderdetail WHERE ipodetid='" . (int)$value . "'");
                    }
                    ...
                    if(isset($data['items']) && count($data['items']) > 0){
						...
						foreach ($data['items'] as $key => $item) {
							...
							if(count($ipodetid_ids) > 0){
                                ...
                                DB::connection('mysql_dynamic')->select($insert_pod_query);
                            }
