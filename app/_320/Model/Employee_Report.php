...
class Employee_Report extends Model
{
    ...
    public function getCategoriesReport($sdate,$edate) {
        ...
        $sql="SELECT trn_s.vusername as vusername, (case when trn_s.vtendertype='Return' then trn_s.vtendertype else trn_s.vtrntype end)as TrnType, trn_sd.isalesid as isalesid, date_format(trn_s.dtrandate,'%m-%d-%Y %H:%i:%s') as trn_date_time, trn_sd.vitemname as vitemname, trn_sd.nextunitprice as nextunitprice, trn_s.iuserid as iuserid, trn_sd.idettrnid as idettrnid, trn_sd.vitemcode as vitemcode, trn_sd.ndebitqty as ndebitqty, trn_sd.nunitprice as nunitprice, 'First Query' as QR 
            ...
            where date_format(mdi.LastUpdate,'%m-%d-%Y') >= '".$data['start_date']."'  AND date_format(mdi.LastUpdate,'%m-%d-%Y') <=  '".$data['end_date']."'
