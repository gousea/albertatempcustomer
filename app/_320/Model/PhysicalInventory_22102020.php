                    ...
                    foreach ($data['items'] as $k => $item) {
                        ...
                        $parent_item = isset($parent_item[0])?(array)$parent_item[0]:[];
                            ...
                            if($parent_item['isparentchild'] == 1){
                                    ...
                                    //trn_itempricecosthistory
                                    DB::connection('mysql_dynamic')->insert("INSERT INTO trn_itempricecosthistory SET  iitemid = '" . $parent_item['iitemid'] . "',vbarcode = '" . ($parent_item['vbarcode']) . "', vtype = 'PhyQOH', noldamt = '" . ($parent_item['iqtyonhand']) . "', nnewamt = '0', iuserid = '" . Auth::user()->id . "', dhistorydate = CURDATE(), thistorytime = CURTIME(),SID = '" . (int)(session()->get('sid'))."'");
                                    //trn_itempricecosthistory
                                    ...
                                    if(count($result)){ 
                                        ...
                                        try{
                                            ...
                                            DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $parent_item['iitemid'] . "',userid = '" . Auth::user()->id . "',barcode = '" . ($parent_item['vbarcode']) . "', type = 'PhyQOH', oldamount = '" . ($parent_item['iqtyonhand']) . "', newamount = '0', general = '" . $x_general . "', source = 'Physical', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
                                        }
                                        catch (QueryException $e) {
                                            Log::error($e);
                                        }
                                    ...
                                    //trn_itempricecosthistory
                                    DB::connection('mysql_dynamic')->insert("INSERT INTO trn_itempricecosthistory SET  iitemid = '" . $parent_master_item['iitemid'] . "',vbarcode = '" . ($parent_master_item['vbarcode']) . "', vtype = 'PhyQOH', noldamt = '" . ($parent_master_item['iqtyonhand']) . "', nnewamt = '" . ($item['itotalunit']) . "', iuserid = '" . Auth::user()->id . "', dhistorydate = CURDATE(), thistorytime = CURTIME(),SID = '" . (int)(session()->get('sid'))."'");
                                    //trn_itempricecosthistory
                                    ...
                                    if(count($result)){
                                        ...
                                        try{
                                            ...
                                            DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $parent_master_item['iitemid'] . "',userid = '" . Auth::user()->id . "',barcode = '" . ($parent_master_item['vbarcode']) . "', type = 'QOH', oldamount = '" . ($parent_master_item['iqtyonhand']) . "', newamount = '". ($item['itotalunit']) ."', source = 'Physical', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
                                        }
                                        catch (QueryException $e) {
                                            Log::error($e);
                                        }
                                    ...
                                    //trn_itempricecosthistory
                                    DB::connection('mysql_dynamic')->insert("INSERT INTO trn_itempricecosthistory SET  iitemid = '" . $parent_item['iitemid'] . "',vbarcode = '" . ($parent_item['vbarcode']) . "', vtype = 'PhyQOH', noldamt = '" . ($parent_item['iqtyonhand']) . "', nnewamt = '" . ($item['itotalunit']) . "', iuserid = '" . Auth::user()->id . "', dhistorydate = CURDATE(), thistorytime = CURTIME(),SID = '" . (int)(session()->get('sid'))."'");
                                    //trn_itempricecosthistory
                                    ...
                                    if(count($result)){
                                        ...
                                        try{
                                            ...
                                            DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $parent_item['iitemid'] . "',userid = '" . Auth::user()->id . "',barcode = '" . ($parent_item['vbarcode']) . "', type = 'QOH', oldamount = '" . ($parent_item['iqtyonhand']) . "', newamount = '". ($item['itotalunit']) ."', source = 'Physical', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
                                        }
                                        catch (QueryException $e) {
                                            Log::error($e);
                                        }
        ...
        if(isset($datas) && count($datas) > 0){
            foreach ($datas as $key => $data) {
               ...
               try {
                    ...
                    $query = "INSERT INTO trn_physicalinventorydetail (ipiid, `vitemid`, vitemname ,`vunitcode` ,`vunitname` , ndebitqty , ncreditqty , ndebitunitprice , ncrediteunitprice, nordtax, ndebitextprice, ncrditextprice, ndebittextprice, ncredittextprice, vbarcode, vreasoncode, ndiffqty, vvendoritemcode, npackqty, nunitcost, itotalunit, SID) VALUES ".implode(',', $items)." ";
