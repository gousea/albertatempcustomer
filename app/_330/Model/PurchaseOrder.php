...
class PurchaseOrder extends Model
{
            ...
            if(count($pre_items_id) > 0){
                ...
                if(isset($search_items['item_name']) && !empty($search_items['item_name'])){
                    ...
                    if(isset($parentid) && !empty($parentid)){
                        ...
                        mi.dcostprice,mi.dunitprice,mi.nunitcost,mi.npack,mi.lot_npack,mi.vitemtype,
                        ...
                        mi.dunitprice,mi.nunitcost,mi.npack,mi.lot_npack,mi.vitemtype,
                        case mi.isparentchild  when 1 then   Mod(p.IQTYONHAND,p.NPACK)  else  
                        (Concat(cast(((mi.IQTYONHAND div mi.NPACK )) as signed), '  (', Mod(mi.IQTYONHAND,mi.NPACK) ,')') )end as QOH,
                        mi.new_costprice FROM mst_item as mi  
                        LEFT JOIN mst_department as md ON(mi.vdepcode = md.vdepcode) 
                ...
                if(isset($search_items['item_name']) && !empty($search_items['item_name'])){
                    ...
                    if(isset($parentid) && !empty($parentid)){
                        ...
                        case mi.isparentchild when 0 then trim(mi.vitemname)  when 1 then Concat(mi.vitemname,' [Child]')
                        when 2 then  Concat(mi.vitemname,' [Parent]') end   as vitemname,
                        mi.vbarcode,mi.nsellunit,mi.vsize,mi.iqtyonhand,mi.dcostprice,mi.dunitprice,mi.nunitcost,mi.npack,mi.lot_npack,mi.vitemtype,
                        case mi.isparentchild  when 1 then   Mod(p.IQTYONHAND,p.NPACK)  else  
                        (Concat(cast(((mi.IQTYONHAND div mi.NPACK )) as signed), '  (', Mod(mi.IQTYONHAND,mi.NPACK) ,')') )end as QOH,
                        mi.new_costprice FROM mst_item as mi  
                        LEFT JOIN mst_department as md ON(mi.vdepcode = md.vdepcode) 
    ...
    {
        if(isset($ivendorid) && !empty($ivendorid)){
            if(count($pre_items_id) > 0){
                $pre_items_id = implode(',', $pre_items_id);
            ...
            if(count($pre_items_id) > 0){
                $pre_items_id = implode(',', $pre_items_id);
                $sql_query = "SELECT mi.iitemid, mi.vitemcode ,mi.vitemname, mi.vbarcode , mi.vsize,mi.iqtyonhand,mi.dcostprice,mi.dunitprice,mi.nunitcost,mi.npack,CASE WHEN mi.npack = 1 or (mi.npack is null)   then mi.iqtyonhand else (Concat(cast(((mi.iqtyonhand div mi.npack )) as signed), '  (', Mod(mi.iqtyonhand,mi.npack) ,')') ) end as QOH, mitv.vvendoritemcode, mitv.ivendorid, mi.new_costprice FROM mst_itemvendor as mitv, mst_item as mi WHERE mitv.iitemid=mi.iitemid AND mitv.vvendoritemcode LIKE  '%" .($search_item). "%' AND mitv.iitemid NOT IN($pre_items_id) AND mi.estatus='Active'";
                // $query = DB::connection('mysql_dynamic')->select($sql_query);
            }else{
                // $query = DB::connection('mysql_dynamic')->select("SELECT mi.iitemid,mi.vitemcode,mi.vitemname,mi.vbarcode,mi.vsize,mi.iqtyonhand,mi.dcostprice,mi.dunitprice,mi.nunitcost,mi.npack,CASE WHEN mi.npack = 1 or (mi.npack is null)   then mi.iqtyonhand else (Concat(cast(((mi.iqtyonhand div mi.npack )) as signed), '  (', Mod(mi.iqtyonhand,mi.npack) ,')') ) end as
                // QOH, mitv.vvendoritemcode,mitv.ivendorid,
                ...
                QOH, mitv.vvendoritemcode,mitv.ivendorid,
                mi.new_costprice FROM mst_itemvendor as mitv ,
                mst_item as mi WHERE mitv.iitemid=mi.iitemid AND mitv.vvendoritemcode='". $search_item ."' AND mi.estatus='Active'";
                ...
                if(count($trn_purchaseorder) > 0){
                    ...
                    if(count($trn_purchaseorderdetail)){
                        foreach ($trn_purchaseorderdetail as $k => $v) {
                            DB::connection('mysql_dynamic')->statement("INSERT INTO mst_delete_table SET  TableName = 'trn_purchaseorderdetail',`Action` = 'delete',`TableId` = '" . (int)$v['ipodetid'] . "',SID = '" . (int)(session()->get('sid'))."'");
                            
                            DB::connection('mysql_dynamic')->statement("DELETE FROM trn_purchaseorderdetail WHERE ipodetid='" . (int)$v['ipodetid'] . "'");
                        }
                    }
                    
                    DB::connection('mysql_dynamic')->statement("DELETE FROM trn_purchaseorder WHERE ipoid='" . (int)$value . "'");
