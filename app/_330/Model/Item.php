...
class Item extends Model
{
    ...
    public function getItem($iitemid)
    {
        ...
        if(count($query) > 0){
            $value = $query[0];
                ...
                $itempacks = DB::connection('mysql_dynamic')->select("SELECT *, 0 as pack_qoh FROM mst_itempackdetail WHERE iitemid='". (int)$value->iitemid ."' ORDER BY ipack DESC");
                ...
                $datas['lot_child_limit'] = $value->lot_child_limit;
                $datas['lot_npack'] = $value->lot_npack;
                $datas['envt_charge'] = $value->envt_charge;
            ...
            if(isset($data['stores_hq'])){
                ...
                for($i=0;$i<count($stores); $i++){
                    try {
                        ...
                        if($data['vitemtype'] == 'Lot Matrix'){
                            ...
                            DB::connection('mysql')->statement("UPDATE u".$stores[$i].".mst_item SET lot_child_limit = '".$data['lot_child_limit']."', iqtyonhand = '".(int)($data['iqtyonhand']*$data['lot_child_limit'])."' WHERE iitemid = '".(int)$last_id ."' "); 
                            
                            $vpackname = 'Single';
                            $vdesc = 'Single';
                            
                            $nunitcost = $data['new_costprice']/(int)$data['lot_child_limit'];
                            if($nunitcost == ''){
                                $nunitcost = 0;
                            }
                            ...
                            $unitprice = $data['dunitprice']/(int)$data['lot_child_limit'];
                            if($data['dunitprice'] == ''){
                                $unitprice = 0;
                            }
                            $npackprice = (int)$ipack * $unitprice;
                            ...
                            $qoh = $data['iqtyonhand']*$data['lot_child_limit'];
                            ...
                            $return = DB::connection('mysql')->select("SELECT idetid FROM u".$stores[$i].".mst_itempackdetail ORDER BY idetid DESC LIMIT 1");
                            $idetid = $return[0]->idetid;
                            
                            $vbarcode = ($data['vbarcode']).'-'.$idetid;
                            DB::connection('mysql')->update("UPDATE u".$stores[$i].".mst_itempackdetail SET `vpackbarcode` = '".$vbarcode."' WHERE idetid = '".(int)$idetid."' ");
                            
                            if(isset($data['lotData']) && !empty($data['lotData'])){
                                
                                $packcost = (int)$data['lotData']['vpack'] * $nunitcost;
                                
                                $packprice = (int)$data['lotData']['npackprice'];
                                
                                $packmargin = $packprice - $packcost;
                                ...
                                $packmargin = (($packmargin/$packprice) * 100);
                                $packmargin = number_format((float)$packmargin, 2, '.', '');
                            
                                DB::connection('mysql')->insert("INSERT INTO u".$stores[$i].".mst_itempackdetail SET  iitemid = '" . (int)$last_id . "', `vbarcode` = '" . $data['vbarcode'] . "', `vpackname` = '" . $data['lotData']['vpackname'] . "',`vdesc` = '" . $data['lotData']['vdesc'] . "',`ipack` = '" . (int)$data['lotData']['vpack'] . "',`iparentid` = '" . (int)$iparentid . "',`npackcost` = '" . $packcost . "',`npackprice` = '" . $data['lotData']['npackprice'] . "',`npackmargin` = '" . $packmargin . "', `isequence` = '" .(int)$data['lotData']['isequence']. "', SID = '" . (int)(session()->get('sid')) . "'");
                ...
                try {
                    ...
                    if($data['vitemtype'] == 'Lot Matrix'){
                        ...
                        //=====commented on 17/08/2021===
                        
                        DB::connection('mysql_dynamic')->statement("UPDATE mst_item SET lot_child_limit = '".$data['lot_child_limit']."', iqtyonhand = '".(int)($data['iqtyonhand']*$data['lot_child_limit'])."' WHERE iitemid = '".(int)$last_id ."' ");
                        ...
                        $return = DB::connection('mysql_dynamic')->select("SELECT idetid FROM mst_itempackdetail ORDER BY idetid DESC LIMIT 1");
                        $idetid = $return[0]->idetid;
                        
                        $vbarcode = ($data['vbarcode']).'-'.$idetid;
                        DB::connection('mysql_dynamic')->update("UPDATE mst_itempackdetail SET `vpackbarcode` = '".$vbarcode."' WHERE idetid = '".(int)$idetid."' ");
            ...
            if(isset($data['stores_hq_lot'])){
                ...
                foreach($data['stores_hq_lot'] as $store){
                    ...
                    try {
                        ...
                        if(count($item_lotmatrix) > 0){
                            ...
                            $item_data = DB::connection('mysql')->select("select * u".$store.".from mst_item where iitemid = '" . (int)($iitem_id) ."' ");
                            
                            $item_data = array_map(function ($value) {
                                return (array)$value;
                            }, $item_data);
                            $item_data = $item_data[0];
                            
                            // $lot_child_count = DB::connection('mysql')->select("select count(idetid) as lot_child_count from u".$store.".mst_itempackdetail WHERE iitemid='". (int)($data['iitemid']) ."'");
                        
                            if($item_data['lot_child_limit'] >= (int)($data['ipack'])){
                                ...
                                $return = DB::connection('mysql')->select("SELECT idetid u".$store.".FROM mst_itempackdetail WHERE iitemid='". (int)($iitem_id) ."' ORDER BY idetid DESC LIMIT 1");
                                $last_id = $return[0]->idetid;
                                
                                $vbarcode = ($data['vbarcode']).'-'.$last_id;
                                DB::connection('mysql')->update("UPDATE u".$store.".mst_itempackdetail SET `vpackbarcode` = '".$vbarcode."' WHERE idetid = '".(int)$last_id."' ");
                            
                                
                                $lot_child_ipack = DB::connection('mysql')->select("select * from u".$store.".mst_itempackdetail WHERE iitemid='". (int)($data['iitemid']) ."' ORDER BY ipack DESC");
                                
                                $total_parent_unit = (int)$item_data['lot_child_limit']*(int)$item_data['iqtyonhand'];
                                
                                $success['success'] = 'Successfully Added Item Pack';
                            }else{
                                $error['error'] = 'New Add Item Pack Limit Reached';
                                return $error;
                            }
                        }
                    }
                ...
                try {
                    ...
                    if(count($item_lotmatrix) > 0){
                        ...
                        $item_data = DB::connection('mysql_dynamic')->select("select * from mst_item where iitemid = '" . (int)($data['iitemid']) ."' ");
                        ...
                        // $lot_child_count = DB::connection('mysql_dynamic')->select("select * from mst_itempackdetail WHERE iitemid='". (int)($data['iitemid']) ."'");
                        ...
                        if($item_data['lot_child_limit'] >= (int)($data['ipack'])){
                            ...
                            $return = DB::connection('mysql_dynamic')->select("SELECT idetid FROM mst_itempackdetail WHERE iitemid='". (int)($data['iitemid']) ."' ORDER BY idetid DESC LIMIT 1");
                            $last_id = $return[0]->idetid;
                            
                            $vbarcode = ($data['vbarcode']).'-'.$last_id;
                            DB::connection('mysql_dynamic')->update("UPDATE mst_itempackdetail SET `vpackbarcode` = '".$vbarcode."' WHERE idetid = '".(int)$last_id."' ");
                            
                            $lot_child_ipack = DB::connection('mysql_dynamic')->select("select * from mst_itempackdetail WHERE iitemid='". (int)($data['iitemid']) ."' ORDER BY ipack DESC");
            ...
            if(isset($stores_hq)){
                ...
                foreach($stores as $store){
                    ...
                    try {
                        ...
                        $item_lotmatrix = DB::connection('mysql_dynamic')->select("SELECT * FROM u".$store.".mst_itempackdetail WHERE iitemid = '".(int)($iitem_id)."' AND idetid !='". (int)($idet_pack_id) ."' AND (vpackname= '". ($datas['vpackname']) ."' OR vpackbarcode = '".$datas['vpackbarcode']."') ");
                    
                        if(count($item_lotmatrix) > 0){
                            $error['error'] = 'Item Pack Or Barcode Already Exist';
                            return $error;
                        }else{
                            ...
                            $sql_query = "UPDATE u".$store.".mst_itempackdetail SET npackcost = '".$datas['npackcost']."', 
                                        ...
                                        vpackbarcode = '".$datas['vpackbarcode']."'
                ...
                try {
                    ...
                    $item_lotmatrix = DB::connection('mysql_dynamic')->select("SELECT * FROM mst_itempackdetail WHERE iitemid = '".(int)($datas['iitemid'])."' AND idetid !='". (int)($datas['idetid']) ."' AND (vpackname= '". ($datas['vpackname']) ."' OR vpackbarcode = '".$datas['vpackbarcode']."') ");
                    // $item_lotmatrix = isset($item_lotmatrix[0])?(array)$item_lotmatrix[0]:[];
                    ...
                    if(count($item_lotmatrix) > 0){
                        ...
                        $sql_query = "UPDATE mst_itempackdetail SET npackcost = '".$datas['npackcost']."', vpackname = '" .$datas['vpackname']. "', vdesc = '" .$datas['vdesc']. "',npackprice = '" .$datas['npackprice']. "',npackmargin = '" .$datas['npackmargin']. "', isequence = '" . (int)($datas['isequence']) . "', vpackbarcode = '".$datas['vpackbarcode']."' WHERE iitemid='". (int)($datas['iitemid']) ."' AND idetid='". (int)($datas['idetid']) ."'";
    ...
    public function deleteItemLotmatrix($data = array(),$iitemid) {
        $success =array();
        $error =array();
        if(isset($data) && count($data) > 0){
            if(isset($data['stores_hq'])){
                foreach($data['data'] as $value){
                    ...
                    foreach($data['stores_hq'] as $store){
                        ...
                        try {
                            ...
                            $item_data = DB::connection('mysql_dynamic')->select("select * from u".$store.".mst_item where iitemid = '" . (int)($iitemid) ."' ");
                            $item_pack_data = DB::connection('mysql_dynamic')->select("select * from u".$store.".mst_itempackdetail WHERE idetid='" . (int)$idetid . "'");
                            
                            if($item_data[0]->lot_child_limit == $item_pack_data[0]->ipack){
                                
                                $warning['error'] = 'You can not delete Max Pack';
                                return $warning;
                            }
                ...
                foreach($data['data'] as $value){
                    try {
                        $item_data = DB::connection('mysql_dynamic')->select("select * from mst_item where iitemid = '" . (int)($iitemid) ."' ");
                        $item_pack_data = DB::connection('mysql_dynamic')->select("select * from mst_itempackdetail WHERE idetid='" . (int)$value . "'");
                    ...
                    try {
                        ...
                        if(isset($data['itemimage']) && !empty($data['itemimage'])){
                            ...
                            if(isset($data['subcat_id']) && !empty($data['subcat_id']) && trim($data['subcat_id']) != '' && trim($data['subcat_id']) != '--Select SubCategory--'){
                                ...
                                // else
                                // {
                                    // $sql_update = "UPDATE u".$value.".mst_item SET  webstore = '" . ($data['webstore']) . "',vitemtype = '" . ($data['vitemtype']) . "',vitemname = '" . ($vitemname) . "',vunitcode = '" . ($data['vunitcode']) . "',  vpricetype = '" . ($data['vpricetype']) . "', vcategorycode = '" .$cat_code. "',subcat_id = '" . $sub_cat_code . "', vdepcode = '" .$department_code. "', vsuppliercode = '" .$supplier_code. "', iqtyonhand = '" . (int)($data['iqtyonhand']) . "', ireorderpoint = '" . (int)($data['ireorderpoint']) . "',reorder_duration = '" . (int)($data['reorder_duration']) . "',manufacturer_id = '" . (int)($data['manufacturer_id']) . "',new_costprice = '" . ($data['new_costprice']) . "', dcostprice = '" . ($data['dcostprice']) . "', dunitprice = '" . ($data['dunitprice']) . "', nsaleprice = '" . ($data['nsaleprice']) . "', nlevel2 = '" . ($data['nlevel2']) . "', nlevel3 = '" . ($data['nlevel3']) . "', nlevel4 = '" . ($data['nlevel4']) . "', iquantity = '" . (int)($data['iquantity']) . "', ndiscountper = '" .(float)$data['ndiscountper'] . "', ndiscountamt = '" . ($data['ndiscountamt']) . "', vtax1 = '" . ($data['vtax1']) . "', vtax2 = '" . ($data['vtax2']) . "', vtax3 = '" . ($data['vtax3']) . "', vfooditem = '" . ($data['vfooditem']) . "', vdescription = '" . ($data['vdescription']) . "', visinventory = '" . ($data['visinventory']) . "', estatus = '" . ($data['estatus']) . "', nbuyqty = '" . (int)($data['nbuyqty']) . "', ndiscountqty = '" . (int)($data['ndiscountqty']) . "', nsalediscountper = '" . ($data['nsalediscountper']) . "', vshowimage = '" . ($data['vshowimage']) . "', itemimage = '" . ($img) . "', vageverify = '" . ($data['vageverify']) . "', ebottledeposit = '" . ($data['ebottledeposit']) . "', nbottledepositamt = '" . ($data['nbottledepositamt']) . "', vbarcodetype = '" . ($data['vbarcodetype']) . "', ntareweight = '" . ($data['ntareweight']) . "', ntareweightper = '" . ($data['ntareweightper']) . "', dlastupdated = '" . ($data['dlastupdated']) . "', nlastcost = '" . ($data['nlastcost']) . "', nonorderqty = '" . (int)($data['nonorderqty']) . "', vparentitem = '" . ($data['vparentitem']) . "', nchildqty = '" . ($data['nchildqty']) . "', vsize = '" . $vsize . "', npack = '" . (int)($data['npack']) . "', nunitcost = '" . ($data['nunitcost']) . "', ionupload = '" . ($data['ionupload']) . "', nsellunit = '" . (int)($data['nsellunit']) . "', ilotterystartnum = '" . (int)($data['ilotterystartnum']) . "', ilotteryendnum = '" . (int)($data['ilotteryendnum']) . "', etransferstatus = '" . ($data['etransferstatus']) . "', vcolorcode = '" . ($data['vcolorcode']) . "', vdiscount = '" . ($data['vdiscount']) . "', norderqtyupto = '" . (int)($data['norderqtyupto']) . "', iinvtdefaultunit = '" . ($data['iinvtdefaultunit']) . "', SID = '" . (int)$value. "', stationid = '" . (int)($data['stationid']) . "', shelfid = '" . (int)($data['shelfid']) . "', aisleid = '" . (int)($data['aisleid']) . "', shelvingid = '" . (int)($data['shelvingid']) . "', PrinterStationId = '" . (int)($data['PrinterStationId']) . "', liability = '" . ($data['liability']) . "', isparentchild = '" . (int)($data['isparentchild']) . "', parentid = '" . (int)($data['parentid']) . "', parentmasterid = '" . (int)($data['parentmasterid']) . "', wicitem = '" . (int)($data['wicitem']) . "', lot_npack = '".(int)$data['lot_npack']."' WHERE iitemid = '" . (int)$previous_item['iitemid'] . "'";
                                // }
                                
                                $sql_update = "UPDATE u".$value.".mst_item SET envt_charge = '" . $data['envt_charge'] . "', webstore = '" . ($data['webstore']) . "',vitemtype = '" . ($data['vitemtype']) . "',vitemname = '" . ($vitemname) . "',vunitcode = '" . ($data['vunitcode']) . "',  vpricetype = '" . ($data['vpricetype']) . "', vcategorycode = '" .$cat_code. "',subcat_id = '" . $sub_cat_code . "', vdepcode = '" .$department_code. "', vsuppliercode = '" .$supplier_code. "', iqtyonhand = '" . (int)($data['iqtyonhand']) . "', ireorderpoint = '" . (int)($data['ireorderpoint']) . "',reorder_duration = '" . (int)($data['reorder_duration']) . "',manufacturer_id = '" . (int)($data['manufacturer_id']) . "',new_costprice = '" . ($data['new_costprice']) . "', dcostprice = '" . ($data['dcostprice']) . "', dunitprice = '" . ($data['dunitprice']) . "', nsaleprice = '" . ($data['nsaleprice']) . "', nlevel2 = '" . ($data['nlevel2']) . "', nlevel3 = '" . ($data['nlevel3']) . "', nlevel4 = '" . ($data['nlevel4']) . "', iquantity = '" . (int)($data['iquantity']) . "', ndiscountper = '" .(float)$data['ndiscountper'] . "', ndiscountamt = '" . ($data['ndiscountamt']) . "', vtax1 = '" . ($data['vtax1']) . "', vtax2 = '" . ($data['vtax2']) . "', vtax3 = '" . ($data['vtax3']) . "', vfooditem = '" . ($data['vfooditem']) . "', vdescription = '" . ($data['vdescription']) . "', visinventory = '" . ($data['visinventory']) . "', estatus = '" . ($data['estatus']) . "', nbuyqty = '" . (int)($data['nbuyqty']) . "', ndiscountqty = '" . (int)($data['ndiscountqty']) . "', nsalediscountper = '" . ($data['nsalediscountper']) . "', vshowimage = '" . ($data['vshowimage']) . "', itemimage = '" . ($img) . "', vageverify = '" . ($data['vageverify']) . "', ebottledeposit = '" . ($data['ebottledeposit']) . "', nbottledepositamt = '" . ($data['nbottledepositamt']) . "', vbarcodetype = '" . ($data['vbarcodetype']) . "', ntareweight = '" . ($data['ntareweight']) . "', ntareweightper = '" . ($data['ntareweightper']) . "', dlastupdated = '" . ($data['dlastupdated']) . "', nlastcost = '" . ($data['nlastcost']) . "', nonorderqty = '" . (int)($data['nonorderqty']) . "', vparentitem = '" . ($data['vparentitem']) . "', nchildqty = '" . ($data['nchildqty']) . "', vsize = '" . $vsize . "', npack = '" . (int)($data['npack']) . "', nunitcost = '" . ($data['nunitcost']) . "', ionupload = '" . ($data['ionupload']) . "', nsellunit = '" . (int)($data['nsellunit']) . "', ilotterystartnum = '" . (int)($data['ilotterystartnum']) . "', ilotteryendnum = '" . (int)($data['ilotteryendnum']) . "', etransferstatus = '" . ($data['etransferstatus']) . "', vcolorcode = '" . ($data['vcolorcode']) . "', vdiscount = '" . ($data['vdiscount']) . "', norderqtyupto = '" . (int)($data['norderqtyupto']) . "', iinvtdefaultunit = '" . ($data['iinvtdefaultunit']) . "', SID = '" . (int)$value. "', stationid = '" . (int)($data['stationid']) . "', shelfid = '" . (int)($data['shelfid']) . "', aisleid = '" . (int)($data['aisleid']) . "', shelvingid = '" . (int)($data['shelvingid']) . "', PrinterStationId = '" . (int)($data['PrinterStationId']) . "', liability = '" . ($data['liability']) . "', isparentchild = '" . (int)($data['isparentchild']) . "', parentid = '" . (int)($data['parentid']) . "', parentmasterid = '" . (int)($data['parentmasterid']) . "', wicitem = '" . (int)($data['wicitem']) . "', lot_npack = '".(int)$data['lot_npack']."' WHERE iitemid = '" . (int)$previous_item['iitemid'] . "'";
                        ...
                        if(($data['vitemtype']) == 'Lot Matrix'){
                            ...
                            DB::connection('mysql')->update("UPDATE u".$value.".mst_item SET lot_child_limit = '".$data['lot_child_limit']."' WHERE iitemid = '".(int)$previous_item['iitemid']."' ");
                            ...
                            if((count($isParentCheck) > 0) && ($isParentCheck['isparentchild'] == 2)){
                                ...
                                $lot_child_items = array_map(function ($lot_child_items) {
                                    return (array)$lot_child_items;
                                }, $lot_child_items);
                            ...
                            if(count($itempackexist) > 0){
                                ...
                                $return = DB::connection('mysql')->select("SELECT idetid FROM u".$value.".mst_itempackdetail ORDER BY idetid DESC LIMIT 1");
                                $idetid = $return[0]->idetid;
                                
                                $vbarcode = ($data['vbarcode']).'-'.$idetid;
                                DB::connection('mysql')->update("UPDATE u".$value.".mst_itempackdetail SET `vpackbarcode` = '".$vbarcode."' WHERE idetid = '".(int)$idetid."' ");
                                ...
                                if(isset($data['lotData']) && !empty($data['lotData'])){
                                    ...
                                    DB::connection('mysql')->insert("INSERT INTO u".$value.".mst_itempackdetail SET  iitemid = '" . (int)$iitemid . "',`vbarcode` = '" . ($data['vbarcode']) . "',`vpackname` = '" . $data['lotData']['vpackname'] . "',`vdesc` = '" . $data['lotData']['vdesc'] . "',`ipack` = '" . (int)$data['lotData']['vpack'] . "',`iparentid` = '" . (int)$iparentid . "',`npackcost` = '" . $packcost . "',`npackprice` = '" . $data['lotData']['npackprice'] . "',`npackmargin` = '" . $packmargin . "', `isequence` = '" .(int)$data['lotData']['isequence']. "', SID = '" . (int)(session()->get('sid')) . "'");
                ...
                try {
                        ...
                        if(isset($data['itemimage']) && !empty($data['itemimage'])){
                            ...
                            if(isset($data['subcat_id']) && !empty($data['subcat_id']) && trim($data['subcat_id']) != '' && trim($data['subcat_id']) != '--Select SubCategory--'){
                                ...
                                if( $sid == "1097")
                                {   
                                    $sql_update = "UPDATE mst_item SET envt_charge = '" . $data['envt_charge'] . "', webstore = '" . ($data['webstore']) . "',`vitemtype` = '" . ($data['vitemtype']) . "',`vitemname` = '" . ($vitemname) . "',`vunitcode` = '" . ($data['vunitcode']) . "', vbarcode = '" . ($data['vbarcode']) . "', vpricetype = '" . ($data['vpricetype']) . "', vcategorycode = '" . ($data['vcategorycode']) . "',subcat_id = '" . ($data['subcat_id']) . "', vdepcode = '" . ($data['vdepcode']) . "', vsuppliercode = '" . ($data['vsuppliercode']) . "', iqtyonhand = '" . (int)($data['iqtyonhand']) . "', ireorderpoint = '" . (int)($data['ireorderpoint']) . "', reorder_duration = '" . (int)($data['reorder_duration']) . "',manufacturer_id = '" . (int)($data['manufacturer_id']) . "',new_costprice = '" . ($data['new_costprice']) . "', dcostprice = '" . ($data['dcostprice']) . "', dunitprice = '" . ($data['dunitprice']) . "', nsaleprice = '" . ($data['nsaleprice']) . "', nlevel2 = '" . ($data['nlevel2']) . "', nlevel3 = '" . ($data['nlevel3']) . "', nlevel4 = '" . ($data['nlevel4']) . "', iquantity = '" . (int)($data['iquantity']) . "', ndiscountper = '" .(float)$data['ndiscountper'] . "', ndiscountamt = '" . ($data['ndiscountamt']) . "', vtax1 = '" . ($data['vtax1']) . "', vtax2 = '" . ($data['vtax2']) . "', vtax3 = '" . ($data['vtax3']) . "', vfooditem = '" . ($data['vfooditem']) . "', vdescription = '" . ($data['vdescription']) . "', visinventory = '" . ($data['visinventory']) . "', estatus = '" . ($data['estatus']) . "', nbuyqty = '" . (int)($data['nbuyqty']) . "', ndiscountqty = '" . (int)($data['ndiscountqty']) . "', nsalediscountper = '" . ($data['nsalediscountper']) . "', vshowimage = '" . ($data['vshowimage']) . "', itemimage = '" . ($img) . "', vageverify = '" . ($data['vageverify']) . "', ebottledeposit = '" . ($data['ebottledeposit']) . "', nbottledepositamt = '" . ($data['nbottledepositamt']) . "', vbarcodetype = '" . ($data['vbarcodetype']) . "', ntareweight = '" . ($data['ntareweight']) . "', ntareweightper = '" . ($data['ntareweightper']) . "', dlastupdated = '" . ($data['dlastupdated']) . "', nlastcost = '" . ($data['nlastcost']) . "', nonorderqty = '" . (int)($data['nonorderqty']) . "', vparentitem = '" . ($data['vparentitem']) . "', nchildqty = '" . ($data['nchildqty']) . "', vsize = '" . ($data['vsize']) . "', npack = '" . (int)($data['npack']) . "', nunitcost = '" . ($data['nunitcost']) . "', ionupload = '" . ($data['ionupload']) . "', nsellunit = '" . (int)($data['nsellunit']) . "', ilotterystartnum = '" . (int)($data['ilotterystartnum']) . "', ilotteryendnum = '" . (int)($data['ilotteryendnum']) . "', etransferstatus = '" . ($data['etransferstatus']) . "', vcolorcode = '" . ($data['vcolorcode']) . "', vdiscount = '" . ($data['vdiscount']) . "', norderqtyupto = '" . (int)($data['norderqtyupto']) . "', iinvtdefaultunit = '" . ($data['iinvtdefaultunit']) . "', SID = '" . (int)(session()->get('sid')) . "', stationid = '" . (int)($data['stationid']) . "', shelfid = '" . (int)($data['shelfid']) . "', aisleid = '" . (int)($data['aisleid']) . "', shelvingid = '" . (int)($data['shelvingid']) . "', PrinterStationId = '" . (int)($data['PrinterStationId']) . "', liability = '" . ($data['liability']) . "', isparentchild = '" . (int)($data['isparentchild']) . "', parentid = '" . (int)($data['parentid']) . "', parentmasterid = '" . (int)($data['parentmasterid']) . "', wicitem = '" . (int)($data['wicitem']) . "', ireorderpointdays = '" . (int)($data['ireorderpointdays']) . "', lot_npack = '".(int)$data['lot_npack']."' WHERE iitemid = '" . (int)$iitemid . "'";
                                }
                                else
                                {
                                    $sql_update = "UPDATE mst_item SET envt_charge = '" . $data['envt_charge'] . "', webstore = '" . ($data['webstore']) . "',`vitemtype` = '" . ($data['vitemtype']) . "',`vitemname` = '" . ($vitemname) . "',`vunitcode` = '" . ($data['vunitcode']) . "', vbarcode = '" . ($data['vbarcode']) . "', vpricetype = '" . ($data['vpricetype']) . "', vcategorycode = '" . ($data['vcategorycode']) . "',subcat_id = '" . ($data['subcat_id']) . "', vdepcode = '" . ($data['vdepcode']) . "', vsuppliercode = '" . ($data['vsuppliercode']) . "', iqtyonhand = '" . (int)($data['iqtyonhand']) . "', ireorderpoint = '" . (int)($data['ireorderpoint']) . "',reorder_duration = '" . (int)($data['reorder_duration']) . "',manufacturer_id = '" . (int)($data['manufacturer_id']) . "',new_costprice = '" . ($data['new_costprice']) . "', dcostprice = '" . ($data['dcostprice']) . "', dunitprice = '" . ($data['dunitprice']) . "', nsaleprice = '" . ($data['nsaleprice']) . "', nlevel2 = '" . ($data['nlevel2']) . "', nlevel3 = '" . ($data['nlevel3']) . "', nlevel4 = '" . ($data['nlevel4']) . "', iquantity = '" . (int)($data['iquantity']) . "', ndiscountper = '" .(float)$data['ndiscountper'] . "', ndiscountamt = '" . ($data['ndiscountamt']) . "', vtax1 = '" . ($data['vtax1']) . "', vtax2 = '" . ($data['vtax2']) . "', vtax3 = '" . ($data['vtax3']) . "', vfooditem = '" . ($data['vfooditem']) . "', vdescription = '" . ($data['vdescription']) . "', visinventory = '" . ($data['visinventory']) . "', estatus = '" . ($data['estatus']) . "', nbuyqty = '" . (int)($data['nbuyqty']) . "', ndiscountqty = '" . (int)($data['ndiscountqty']) . "', nsalediscountper = '" . ($data['nsalediscountper']) . "', vshowimage = '" . ($data['vshowimage']) . "', itemimage = '" . ($img) . "', vageverify = '" . ($data['vageverify']) . "', ebottledeposit = '" . ($data['ebottledeposit']) . "', nbottledepositamt = '" . ($data['nbottledepositamt']) . "', vbarcodetype = '" . ($data['vbarcodetype']) . "', ntareweight = '" . ($data['ntareweight']) . "', ntareweightper = '" . ($data['ntareweightper']) . "', dlastupdated = '" . ($data['dlastupdated']) . "', nlastcost = '" . ($data['nlastcost']) . "', nonorderqty = '" . (int)($data['nonorderqty']) . "', vparentitem = '" . ($data['vparentitem']) . "', nchildqty = '" . ($data['nchildqty']) . "', vsize = '" . ($data['vsize']) . "', npack = '" . (int)($data['npack']) . "', nunitcost = '" . ($data['nunitcost']) . "', ionupload = '" . ($data['ionupload']) . "', nsellunit = '" . (int)($data['nsellunit']) . "', ilotterystartnum = '" . (int)($data['ilotterystartnum']) . "', ilotteryendnum = '" . (int)($data['ilotteryendnum']) . "', etransferstatus = '" . ($data['etransferstatus']) . "', vcolorcode = '" . ($data['vcolorcode']) . "', vdiscount = '" . ($data['vdiscount']) . "', norderqtyupto = '" . (int)($data['norderqtyupto']) . "', iinvtdefaultunit = '" . ($data['iinvtdefaultunit']) . "', SID = '" . (int)(session()->get('sid')) . "', stationid = '" . (int)($data['stationid']) . "', shelfid = '" . (int)($data['shelfid']) . "', aisleid = '" . (int)($data['aisleid']) . "', shelvingid = '" . (int)($data['shelvingid']) . "', PrinterStationId = '" . (int)($data['PrinterStationId']) . "', liability = '" . ($data['liability']) . "', isparentchild = '" . (int)($data['isparentchild']) . "', parentid = '" . (int)($data['parentid']) . "', parentmasterid = '" . (int)($data['parentmasterid']) . "', wicitem = '" . (int)($data['wicitem']) . "', lot_npack = '".(int)$data['lot_npack']."' WHERE iitemid = '" . (int)$iitemid . "'";
                                }
                            }else{
                                $sid = session()->get('sid');
                                if( $sid == "1097")
                        ...
                        if(($data['vitemtype']) == 'Lot Matrix'){
                            ...
                            DB::connection('mysql_dynamic')->update("UPDATE mst_item SET lot_child_limit = '".$data['lot_child_limit']."' WHERE iitemid = '".(int)$previous_item['iitemid']."' ");
                            ...
                            //===This is commented because itempack data is not posted from the side of view still 
                            //====this one update only the first pack of item===11/06/2020====
                            if(count($itempackexist) > 0){
                                // DB::connection('mysql_dynamic')->select("UPDATE mst_itempackdetail SET  `vpackname` = '" . $vpackname . "',`vdesc` = '" . $vdesc . "',`ipack` = '" . (int)$ipack . "',`npackcost` = '" . $npackcost . "',`nunitcost` = '" . $nunitcost . "',`npackprice` = '" . $npackprice . "',`npackmargin` = '" . $npackmargin . "' WHERE vbarcode='". ($data['vbarcode']) ."' AND iitemid='". (int)$iitemid ."' AND iparentid=1");
                            }else{
                                ...
                                if(isset($data['lotData']) && !empty($data['lotData'])){
                                    ...
                                    DB::connection('mysql_dynamic')->insert("INSERT INTO mst_itempackdetail SET  iitemid = '" . (int)$iitemid . "',`vbarcode` = '" . ($data['vbarcode']) . "',`vpackname` = '" . $data['lotData']['vpackname'] . "',`vdesc` = '" . $data['lotData']['vdesc'] . "',`ipack` = '" . (int)$data['lotData']['vpack'] . "',`iparentid` = '" . (int)$iparentid . "',`npackcost` = '" . $packcost . "',`npackprice` = '" . $data['lotData']['npackprice'] . "',`npackmargin` = '" . $packmargin . "', `isequence` = '" .(int)$data['lotData']['isequence']. "', SID = '" . (int)(session()->get('sid')) . "'");
