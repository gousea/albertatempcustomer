    ...
    public function addParentItem($data = array()) 
    {
      ...
      if(isset($data) && count($data) > 0){
          ...
          try {
              ...
              //new functinality unit per case and seeling unit 1(one)
              
              $sellingunitonepaarent=DB::connection('mysql_dynamic')->select("SELECT nsellunit FROM mst_item WHERE iitemid= '". (int)($data['parent_item_id']) ."' ");
             
              $unitpercaseoneparent=DB::connection('mysql_dynamic')->select("SELECT npack FROM mst_item WHERE iitemid= '". (int)($data['parent_item_id']) ."' ");
              
              $parentsellingunit=$sellingunitonepaarent[0]->nsellunit;
              $parentunotpercase=$unitpercaseoneparent[0]->npack;
              ...
              DB::connection('mysql_dynamic')->insert("INSERT INTO trn_physicalinventory SET  vrefnumber= $vrefnumber, estatus='Close',
              dcreatedate=CURRENT_TIMESTAMP, vtype = 'Perent Update',SID = '" . (int)(session()->get('sid'))."'");
              ...
              //perent Qoh Update
              //dd($parentunotpercase);
            //  if($parentsellingunit==1 && $parentunotpercase==1){
             ...
             // dd($parentunotpercase);
              ...
              DB::connection('mysql_dynamic')->insert("INSERT INTO trn_physicalinventory SET  vrefnumber= $vrefnumber,estatus='Close'
              ,dcreatedate=CURRENT_TIMESTAMP, vtype = 'Child Update',SID = '" . (int)(session()->get('sid'))."'");
              ...
              $ipiid = $query1['ipiid'];
                ...
                /// conv upadte $new_quatity_on_hand=($parent_item['iqtyonhand']*$data['cnpack'])+$child_item['iqtyonhand'];
            // $con_quatity_on_hand=($parent_item['iqtyonhand']*$data['cnpack'])-$parent_item['iqtyonhand'];
              ...
              DB::connection('mysql_dynamic')->insert("INSERT INTO trn_physicalinventory SET 
              vrefnumber= $vrefnumber,estatus='Close',dcreatedate=CURRENT_TIMESTAMP, vtype = 'Child Update',SID = '" . (int)(session()->get('sid'))."'");
              ...
              $ipiid = $query1['ipiid'];
                ...
                ///conversion end  toatal adj start
            if($parentsellingunit==1 && $parentunotpercase==1){    
               $total_quatity_on_hand1=($parent_item['iqtyonhand']*$data['cnpack'])+$child_item['iqtyonhand'];
               $total_quatity_on_hand= $total_quatity_on_hand1-($parent_item['iqtyonhand']+$child_item['iqtyonhand']);
              $query = DB::connection('mysql_dynamic')->select("SELECT ipiid FROM trn_physicalinventory ORDER BY ipiid DESC LIMIT 1");
              $query = isset($query[0])?(array)$query[0]:[];
              ...
              DB::connection('mysql_dynamic')->insert("INSERT INTO trn_physicalinventory SET  
              vrefnumber= $vrefnumber,estatus='Close',dcreatedate=CURRENT_TIMESTAMP, vtype = 'Parent Adj',SID = '" . (int)(session()->get('sid'))."'");
              ...
              $ipiid = $query1['ipiid'];
                ...
                DB::connection('mysql_dynamic')->select("INSERT INTO trn_physicalinventorydetail SET  ipiid = '" . (int)$ipiid . "',
                                  ...
                                  vitemname = '" .$parent_item['vitemname'] . "',
                                  vunitcode = '" . $parent_item['vunitcode'] . "',
                                  ndebitqty= '" . $total_quatity_on_hand. "', 
                                  vbarcode = '" . $parent_item['vbarcode']. "', 
                                  SID = '" . (int)(session()->get('sid'))."'
                ");
            }
               ...
               //  $total_quatity_on_hand1=($parent_item['iqtyonhand']*$data['cnpack'])+$child_item['iqtyonhand'];
               $total_quatity_on_hand= $parent_item['iqtyonhand']+$child_item['iqtyonhand'];
              $query = DB::connection('mysql_dynamic')->select("SELECT ipiid FROM trn_physicalinventory ORDER BY ipiid DESC LIMIT 1");
              $query = isset($query[0])?(array)$query[0]:[];
            ...
            //   if( $parentsellingunit==1 && $parentunotpercase==1)
            //  {
            //      $new_quatity_on_hand_child=($parent_item['iqtyonhand']*$data['cnpack'])+$child_item['iqtyonhand'];
            //      DB::connection('mysql_dynamic')->update("UPDATE mst_item SET  dcostprice='". ($parent_item['dcostprice']) ."',nunitcost='". ($parent_item['nunitcost']) ."', npack= '". (int)($data['cnpack']) ."',iqtyonhand='".$new_quatity_on_hand_child."', isparentchild = 1,parentid = '". ($data['parent_item_id']) ."',parentmasterid = '". ($data['parent_item_id']) ."' WHERE iitemid= '". (int)($data['child_item_id']) ."'");
            //  }
            //  else{
            ...
            //  }
             ...
             if( $parentsellingunit==1 && $parentunotpercase==1)
               {
                 $new_quatity_on_hand=($parent_item['iqtyonhand']*$data['cnpack'])+$child_item['iqtyonhand'];
                 
                 DB::connection('mysql_dynamic')->select("UPDATE mst_item SET  iqtyonhand = '". (int)($new_quatity_on_hand) ."', npack= '". (int)($data['cnpack']) ."',nsellunit='". (int)($data['cnpack']) ."'  WHERE iitemid= '". (int)($data['parent_item_id']) ."'");
              }
              else{
     ...
     if(isset($data) && count($data) > 0){
         ...
         try {
            ...
            //update QOH of child item 
              $parent_item = DB::connection('mysql_dynamic')->select("SELECT * FROM mst_item WHERE iitemid= '". (int)($data['selected_parent_item_id']) ."' ");
              $parent_item = isset($parent_item[0])?(array)$parent_item[0]:[];

             $quatity_on_hand_child =  $parent_item['iqtyonhand'];
             //'". (int)($quatity_on_hand) ."'
            
             DB::connection('mysql_dynamic')->update("UPDATE mst_item SET iqtyonhand = '". (int)($quatity_on_hand_child) ."',isparentchild=0,parentid=0,parentmasterid=0 WHERE iitemid= '". (int)($data['iitemid']) ."'");
           ...
           //DB::connection('mysql_dynamic')->update("UPDATE mst_item SET isparentchild=0,parentid=0,parentmasterid=0 WHERE iitemid= '". (int)($data['iitemid']) ."'");
            ...
            if($remove_parent_item_update_list['isparentchild'] == 2){
                ...
                DB::connection('mysql_dynamic')->update("UPDATE mst_item SET isparentchild=0,parentid=0,parentmasterid=0, nsellunit=1, npack=1 WHERE iitemid= '". (int)($data['selected_parent_item_id']) ."'");
            }
         }
