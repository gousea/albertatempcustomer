        ...
        if(isset($datas) && count($datas) > 0){
            ...
            foreach ($datas as $key => $data) {//vpackname vdesc
                if(isset($stores_hq)){
                    ...
                    foreach($stores as $store){
                        $pack_idet_id = DB::connection('mysql_dynamic')->select("select * from mst_itempackdetail where  idetid= '".(int)$data['idetid']."' ");
                        foreach($pack_idet_id as $pack_id){
                           $idet_id =  $pack_id->idetid;
                           $vpackname = $pack_id->vpackname;
                           $vbarcode = $pack_id->vbarcode;
                        }
                        $item_data = DB::connection('mysql_dynamic')->select("select * from mst_item where iitemid = '".(int)$data['iitemid']."' ");
                        foreach($item_data as $item){
                            $item_vbarcode = $item->vbarcode;
                        }
                        
                        $item_data_ids = DB::connection('mysql')->select("select * from u".$store.".mst_item where  vbarcode = '".$item_vbarcode."' ");
                        // dd($item_data_ids);
                        foreach($item_data_ids as $item_ids){
                            $iitem_id = $item_ids->iitemid;
                        }
                        // dd($iitem_id);
                        $item_lotmatrix = DB::connection('mysql')->select("SELECT * FROM u".$store.".mst_itempackdetail WHERE iitemid = '".(int)($iitem_id)."' AND vpackname= '".$vpackname."'  ");
                        // dd($item_lotmatrix);
                        foreach($item_lotmatrix as $i_pack_id){
                           $idet_pack_id =  $i_pack_id->idetid;
                        }
                        
                        
                        try {
                            $sql_query = "UPDATE u".$store.".mst_itempackdetail SET npackcost = '".($data['npackcost'])."', vpackname = '" . ($data['vpackname']) . "', vdesc = '" . ($data['vdesc']) . "',npackprice = '" . ($data['npackprice']) . "',npackmargin = '" . ($data['npackmargin']) . "' WHERE iitemid = '".(int)($iitem_id)."' AND idetid= '".(int)($idet_pack_id)."' ";
                ...
                foreach($data['data'] as $value){
                    ...
                    foreach($data['stores_hq'] as $store){
                        ...
                        try {
                            ...
                            DB::connection('mysql')->select("DELETE FROM u".$store.".mst_itempackdetail WHERE idetid='" . (int)$idetid . "'");
                        }
                        catch (QueryException $e) {
                            // not a MySQL exception
                            $error['error'] = $e->getMessage(); 
