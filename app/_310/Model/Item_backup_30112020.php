...
class Item extends Model
{
                ...
                for($i=0;$i<count($srotes_hq); $i++){
                    ...
                    for($j=0; $j < count($data); $j++ ){
                            if($data[$j] > 0 && $data[$j] < 100){ 
                                ...
                                // $inser_query = "INSERT INTO u".$srotes_hq[$i].".mst_delete_table SET  TableName = 'mst_item',`Action` = 'delete',`TableId` = '" . (int)$data[$j] . "',SID = '" . (int)($sid)."'";
                                
                                $insert_query = "INSERT INTO u".$srotes_hq[$i].".mst_delete_table( TableName, TableId, Action, SID) VALUES('mst_item',  '" . (int)$data[$j] . "', 'delete', '" . (int)($sid)."') ";
                                
                                $a = DB::connection('mysql')->insert($insert_query);
                                
                                $sku = self::where('iitemid', (int)$data[$j])->get('vbarcode');
                                for($z=0; $z < count($sku); $z++){
                                    $vbarcode = $sku[$z]->vbarcode;
                                }
                                ...
                                $del_query = "DELETE FROM u".$srotes_hq[$i].".mst_itemalias WHERE vsku = '".$vbarcode."' ";
    ...
    public function addDepartment($sid, $depid=''){
        if(isset($depid)){
            ...
            foreach($get_dep_name as $dep){
                $dep_name = $dep->vdepartmentname;
            ...
            if(count($checkExists) > 0){
                foreach($checkExists as $depats){
                    return $depats->vdepcode;
                }
            }else{
    ...
    public function addCategory($sid, $catid=''){
        if(isset($catid)){
            ...
            foreach($get_cat_name as $cat){
                $cat_name = $cat->vcategoryname;
                $cat_desc = $cat->vdescription;
                $cat_type = $cat->vcategorttype;
                $cat_isequence = $cat->isequence;
                $cat_status = $cat->estatus;
                $cat_dep_code = $cat->dept_code;
            }
            
            $checkExists = DB::connection('mysql')->select("select * from u".$sid.".mst_category where  vcategoryname = '".$cat_name."' ");
            if(count($checkExists) > 0){
                foreach($checkExists as $cats){
                    return $cats->vcategorycode;
                }
            }else {
                ...
                $inserting = DB::connection("mysql")->select("Insert INTO u".$sid.".mst_category (vcategoryname, vdescription, vcategorttype, isequence, estatus, dept_code, SID) VAlues('".$cat_name."', '".$cat_desc."', '".$cat_type."',  '".$cat_isequence."', '".$cat_status."', '".$depcode."', '".$sid."') ");
                ...
                $updating = DB::connection("mysql")->select("UPDATE u".$sid.".mst_category SET vcategorycode = '".$vcategorycode."'  WHERE icategoryid = '".$vcategorycode."' ");
    ...
    public function addSubcategory($sid, $subcatid=''){
        if(isset($subcatid)){
            ...
            foreach($get_subcat_name as $subcat){
                $sub_cat_name = $subcat->subcat_name;
                $sub_cat_status = $subcat->status;
            ...
            if(count($checkExists) > 0){
                foreach($checkExists as $subcats){
                    return $subcats->subcat_id;
                } 
            }else {
                ...
                if(count($checkCateogory) > 0){
                    foreach($checkCateogory as $cats){
                         $new_cat_id = $cats->icategoryid;
                    }
                    ...
                    DB::connection('mysql')->select($insert_query);
                    ...
                    $inserting = DB::connection("mysql")->select("Insert INTO u".$sid.".mst_category (vcategoryname, vdescription, vcategorttype, isequence, estatus, dept_code, SID) 
    ...
    public function addSupplier($sid, $supplierid=''){
        if(isset($supplierid)){
            ...
            foreach($supplier as $sup){
                $vcompanyname   = $sup->vcompanyname;
                $vvendortype    = $sup->vvendortype;
                $vfnmae         = $sup->vfnmae;
                $vlname         = $sup->vlname;
                $vcode          = $sup->vcode;
                $vaddress1      = $sup->vaddress1; 
                $vcity          = $sup->vcity;
                $vstate         = $sup->vstate;
                $vzip           = $sup->vzip;
                $vcountry       = $sup->vcountry;
                $vphone         = $sup->vphone;
                $vemail         = $sup->vemail;
                $estatus        = $sup->estatus;
                $plcbtype       = $sup->plcbtype;
                $edi            = $sup->edi;
            }
            $checkExists = DB::connection("mysql")->select("select * from u".$sid.".mst_supplier where vcompanyname = '".$vcompanyname."' ");
            
            if(count($checkExists) > 0){
                foreach($checkExists as $sups){
                    return $sups->isupplierid;
                }
            }else{
                ...
                foreach($sup_id as $sups){
                   $isuppliercode = $sups->isupplierid; 
                }
        ...
        if(isset($data) && count($data) > 0){
            if(isset($data['stores_hq'])){
                ...
                try {
                    ...
                    //=============20-11-2020========
                    DB::connection('mysql_dynamic')->insert("INSERT INTO mst_itemvendor SET  iitemid = '" . (int)$last_id . "',`ivendorid` = '" . (int)$data['vsuppliercode'] . "',`vvendoritemcode` = '" . $last_id . "', SID = '" . (int)(session()->get('sid')) . "'");
        ...
        if(isset($data) && count($data) > 0){
            ...
            foreach($data as $value){
                try {
                    ...
                    DB::connection('mysql_dynamic')->select("DELETE FROM mst_itemalias WHERE iitemaliasid='" . (int)$value . "'");
                }
    ...
    public function editlistLotMatrixItems($datas = array()) {
        ...
        if(isset($datas) && count($datas) > 0){
            ...
            foreach ($datas as $key => $data) {//vpackname vdesc
                
                try {
                    $sql_query = "UPDATE mst_itempackdetail SET `npackcost` = '".($data['npackcost'])."', `vpackname` = '" . ($data['vpackname']) . "', `vdesc` = '" . ($data['vdesc']) . "',`npackprice` = '" . ($data['npackprice']) . "',`npackmargin` = '" . ($data['npackmargin']) . "' WHERE iitemid='". (int)($data['iitemid']) ."' AND idetid='". (int)($data['idetid']) ."'";
                    // print_r($sql_query);
        ...
        if(isset($data) && count($data) > 0){
            ...
            foreach($data as $value){
                try {
                    ...
                    DB::connection('mysql_dynamic')->select("DELETE FROM mst_itempackdetail WHERE idetid='" . (int)$value . "'");
                }
    ...
    //                 // dd($value);
    //                 $previous_item = DB::connection('mysql')->select("SELECT * FROM u".$value.".mst_item WHERE vbarcode='" . (int)$data['vbarcode'] . "'");
    ...
    //                             try{
    //                                 DB::connection('mysql')->insert("INSERT INTO u".$value.".trn_webadmin_history SET  itemid = '" . $previous_item['iitemid'] . "',userid = '" . Auth::user()->iuserid . "', barcode = '" . $previous_item['vbarcode'] . "', type = 'All', oldamount = '0', newamount = '0', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)$value."'");
    //                             }
    //                             catch (QueryException $e) {
    //                                 Log::error($e);
    //                             }
    ...
    //                                 try{
    //                                     DB::connection('mysql')->insert("INSERT INTO u".$value.".trn_webadmin_history SET  itemid = '" . $previous_item['iitemid'] . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . ($previous_item['vbarcode']) . "', type = 'Cost', oldamount = '" . $previous_item['dcostprice'] . "', newamount = '" . ($data['dcostprice']) . "', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)$value."'");
    //                                 }
    //                                 catch (QueryException $e) {
    //                                     Log::error($e);
    //                                 }
    ...
    //                                 $x_general_cost->old_item_values = $old_item_values;
    //                                 try{
    //                                     DB::connection('mysql')->insert("INSERT INTO u".$value.".trn_webadmin_history SET  itemid = '" . $previous_item['iitemid'] . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . ($previous_item['vbarcode']) . "', type = 'Cost', oldamount = '" . $previous_item['new_costprice'] . "', newamount = '" . ($data['new_costprice']) . "', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)$value."'");
    //                                 }
    //                                 catch (QueryException $e) {
    //                                     Log::error($e);
    //                                 }
    ...
    //                                 try{
    //                                     DB::connection('mysql')->insert("INSERT INTO u".$value.".trn_webadmin_history SET  itemid = '" . $previous_item['iitemid'] . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . $previous_item['vbarcode'] . "', type = 'Price', oldamount = '" . $previous_item['dunitprice'] . "', newamount = '" . $data['dunitprice'] . "', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)$value."'");
    //                                 }
    //                                 catch (QueryException $e) {
    //                                     Log::error($e);
    //                                 }
    ...
    //                         $x_general->old_item_values = $old_item_values;
    //                         try{
    //                             DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $iitemid . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . $data['vbarcode'] . "', type = 'All', oldamount = '0', newamount = '0', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
    //                         }
    //                         catch (QueryException $e) {
    //                             Log::error($e);
    //                         }
    ...
    //                             $x_general_cost->old_item_values = $old_item_values;
    //                             try{
    //                                 DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $iitemid . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . ($data['vbarcode']) . "', type = 'Cost', oldamount = '" . $previous_item['new_costprice'] . "', newamount = '" . ($data['new_costprice']) . "', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
    //                             }
    //                             catch (QueryException $e) {
    //                                 Log::error($e);
    //                             }
    ...
    //                             $x_general_price->old_item_values = $old_item_values;
    //                             try{
    //                                 DB::connection('mysql_dynamic')->insert("INSERT INTO trn_webadmin_history SET  itemid = '" . $iitemid . "',userid = '" . Auth::user()->iuserid . "',barcode = '" . $data['vbarcode'] . "', type = 'Price', oldamount = '" . $previous_item['dunitprice'] . "', newamount = '" . $data['dunitprice'] . "', source = 'ItemEdit', historydatetime = NOW(),SID = '" . (int)(session()->get('sid'))."'");
    //                             }
    //                             catch (QueryException $e) {
    //                                 Log::error($e);
    //                             }
    ...
    //                         }
    //                     }
    //                     $result = DB::connection('mysql')->update($sql_update);
    ...
    public function addImportItems($data = array())
    {
        if(count($data) > 0){
            ...
            DB::connection('mysql_dynamic')->insert("INSERT INTO mst_item SET  dlastupdated = '" . ($data['dlastupdated']) . "',dcreated = '" . ($data['dcreated']) . "',vbarcode = '" . ($data['vbarcode']) . "',vitemcode = '" . ($data['vitemcode']) . "',vitemname = '" . ($data['vitemname']) . "',vitemtype = '" . ($data['vitemtype']) . "',vcategorycode = '" . ($data['vcategorycode']) . "',vdepcode = '" . ($data['vdepcode']) . "',estatus = '" . ($data['estatus']) . "',dunitprice = '" . ($data['dunitprice']) . "',dcostPrice = '" . ($data['dcostPrice']) . "',vunitcode = '" . ($data['vunitcode']) . "',vtax1 = '" . ($data['vtax1']) . "',vtax2 = '" . ($data['vtax2']) . "',vfooditem = '" . ($data['vfooditem']) . "',vsuppliercode = '" . ($data['vsuppliercode']) . "',vdescription = '" . ($data['vdescription']) . "',vshowimage = '" . ($data['vshowimage']) . "',iquantity = '" . ($data['iquantity']) . "',ireorderpoint = '" . ($data['ireorderpoint']) . "',reorder_duration = '" . ($data['reorder_duration']) . "',iqtyonhand = '" . ($data['iqtyonhand']) . "',npack = '" . ($data['npack']) . "',nunitcost = '" . ($data['nunitcost']) . "',vsize = '" . ($data['vsize']) . "',ionupload = '" . ($data['ionupload']) . "',vcolorcode = '" . ($data['vcolorcode']) . "',vageverify = '". ($data['vageverify']) . "',SID = '" . (int)(session()->get('sid')) . "',visinventory ='Yes',liability = 'N'");
        }
    }
    ...
    public function editLotteryItems($iitemid, $data) {
        ...
        if(isset($data) && count($data) > 0){
              ...
              try {
                    ...
                    // else
                    // {
                    //     $sql_update = "UPDATE mst_item SET  `vitemtype` = '" . ($data['vitemtype']) . "',`vitemname` = '" . ($data['vitemname']) . "',`vunitcode` = '" . ($data['vunitcode']) . "', vbarcode = '" . ($data['vbarcode']) . "', vpricetype = '" . ($data['vpricetype']) . "', vcategorycode = '" . ($data['vcategorycode']) . "',subcat_id = '" . ($data['subcat_id']) . "', vdepcode = '" . ($data['vdepcode']) . "', vsuppliercode = '" . ($data['vsuppliercode']) . "', iqtyonhand = '" . (int)($data['iqtyonhand']) . "', ireorderpoint = '" . (int)($data['ireorderpoint']) . "',reorder_duration = '" . (int)($data['reorder_duration']) . "',manufacturer_id = '" . (int)($data['manufacturer_id']) . "', dcostprice = '" . ($data['dcostprice']) . "', dunitprice = '" . ($data['dunitprice']) . "', nsaleprice = '" . ($data['nsaleprice']) . "', nlevel2 = '" . ($data['nlevel2']) . "', nlevel3 = '" . ($data['nlevel3']) . "', nlevel4 = '" . ($data['nlevel4']) . "', iquantity = '" . (int)($data['iquantity']) . "', ndiscountper = '" . ($data['ndiscountper']) . "', ndiscountamt = '" . ($data['ndiscountamt']) . "', vtax1 = '" . ($data['vtax1']) . "', vtax2 = '" . ($data['vtax2']) . "',  vtax3 = '" . ($data['vtax3']) . "', vfooditem = '" . ($data['vfooditem']) . "', vdescription = '" . ($data['vdescription']) . "', dlastsold = '" . ($data['dlastsold']) . "', visinventory = '" . ($data['visinventory']) . "', dpricestartdatetime = '" . ($data['dpricestartdatetime']) . "', dpriceenddatetime = '" . ($data['dpriceenddatetime']) . "', estatus = '" . ($data['estatus']) . "', nbuyqty = '" . (int)($data['nbuyqty']) . "', ndiscountqty = '" . (int)($data['ndiscountqty']) . "', nsalediscountper = '" . ($data['nsalediscountper']) . "', vshowimage = '" . ($data['vshowimage']) . "', vageverify = '" . ($data['vageverify']) . "', ebottledeposit = '" . ($data['ebottledeposit']) . "', nbottledepositamt = '" . ($data['nbottledepositamt']) . "', vbarcodetype = '" . ($data['vbarcodetype']) . "', ntareweight = '" . ($data['ntareweight']) . "', ntareweightper = '" . ($data['ntareweightper']) . "', dlastupdated = '" . ($data['dlastupdated']) . "', dlastreceived = '" . ($data['dlastreceived']) . "', dlastordered = '" . ($data['dlastordered']) . "', nlastcost = '" . ($data['nlastcost']) . "', nonorderqty = '" . (int)($data['nonorderqty']) . "', vparentitem = '" . ($data['vparentitem']) . "', nchildqty = '" . ($data['nchildqty']) . "', vsize = '" . ($data['vsize']) . "', npack = '" . (int)($data['npack']) . "', nunitcost = '" . ($data['nunitcost']) . "', ionupload = '" . ($data['ionupload']) . "', nsellunit = '" . (int)($data['nsellunit']) . "', ilotterystartnum = '" . (int)($data['ilotterystartnum']) . "', ilotteryendnum = '" . (int)($data['ilotteryendnum']) . "', etransferstatus = '" . ($data['etransferstatus']) . "', vsequence = '" . ($data['vsequence']) . "', itemimage =null , vcolorcode = '" . ($data['vcolorcode']) . "', vdiscount = '" . ($data['vdiscount']) . "', norderqtyupto = '" . (int)($data['norderqtyupto']) . "', vshowsalesinzreport = '" . ($data['vshowsalesinzreport']) . "', iinvtdefaultunit = '" . ($data['iinvtdefaultunit']) . "', SID = '" . (int)($this->session->data['sid']) . "', stationid = '" . (int)($data['stationid']) . "', shelfid = '" . (int)($data['shelfid']) . "', aisleid = '" . (int)($data['aisleid']) . "', shelvingid = '" . (int)($data['shelvingid']) . "', rating = '" . ($data['rating']) . "', vintage = '" . ($data['vintage']) . "', PrinterStationId = '" . (int)($data['PrinterStationId']) . "', liability = '" . ($data['liability']) . "', isparentchild = '" . (int)($data['isparentchild']) . "', parentid = '" . (int)($data['parentid']) . "', parentmasterid = '" . (int)($data['parentmasterid']) . "', wicitem = '" . (int)($data['wicitem']) . "' WHERE iitemid = '" . (int)$iitemid . "'";
                    // }
                    // dd($sql_update);
