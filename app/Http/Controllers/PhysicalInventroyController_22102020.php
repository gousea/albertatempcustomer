...
class PhysicalInventroyController extends Controller
{
        ...
        if ($request->isMethod('post')) {
			...
			if(isset($input['ipiid']) && $input['ipiid'] != ''){
                ...
                foreach ($items as $k => $item) 
                {
					...
					$sql[] = "( '" . (int)$ipiid . "', '" . ($itemdetail['iitemid']) . "', \"" . ($itemdetail['vitemname']) . "\", ' ', ' ', '" . ($item['ndebitqty']) . "', '0.00', '0.00', '0.00', '0.00', '" . ($ndebitextprice) . "', '0.00', '0.00', '0.00', '" . ($itemdetail['vbarcode']) . "', ' ', '" . ($ndiffqty) . "', ' ', '" . ($itemdetail['npack']) . "', '" . ($itemdetail['nunitcost']) . "', '" . ($itotalunit) . "', '" . (int)(session()->get('sid'))."' )";
    ...
    public function edit_calculate(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $sql = "UPDATE trn_physicalinventorydetail SET ndebitqty = (CASE vitemid ".$sql_ndebitqty." END), ndebitextprice = (CASE vitemid ".$sql_ndebitextprice." END), ndiffqty = (CASE vitemid ".$sql_ndiffqty." END), itotalunit = (CASE vitemid ".$sql_itotalunit." END) WHERE vitemid IN('".$arr_vitemid."') AND ipiid = ".$ipiid." ";
			}else{
			    $sql='';
			}
            unset($results);
    ...
    public function display_selected_data(Request $request)
    {
        ...
        if($check['count'] >= 1){
            ...
            if($check_status['estatus'] == 'Open'){
                ...
                $select_query = "SELECT mpis.iitemid, mpis.npack, mpis.vbarcode, mpis.iqtyonhand, mi.vitemname, mi.vitemcode, mi.dunitprice, (mi.dcostprice/mi.npack) as unitcost, tpd.ndebitqty FROM mst_physical_inventory_snapshot as mpis LEFT JOIN mst_item as mi ON(mpis.iitemid = mi.iitemid) LEFT JOIN trn_physicalinventorydetail as tpd ON(mpis.iitemid = tpd.vitemid AND mpis.ipiid = tpd.ipiid) WHERE mpis.ipiid = '".$ipiid."' ORDER BY mpis.vbarcode LIMIT ". $input['start'].", ".$limit;
                $itemdata = DB::connection('mysql_dynamic')->select($select_query);
		...
		if ($request->isMethod('post')) {
			if(isset($input['import_physical_inventory_file']) && $request->hasFile('import_physical_inventory_file')){
			    ...
			    if(in_array($request->file('import_physical_inventory_file')->getClientMimeType(), $filetype)){
                    ...
                    $dsn  = "mysql:host=".\Session::get('dbhost').";dbname=".$request->session()->get('dbname');
        ...
        if ($request->isMethod('post')) {
			...
			if(isset($input['ipiid']) && $input['ipiid'] != ''){
				...
				// echo $k." "; print_r($vbarcode[$k]); echo "<br>";
				    ...
				    $ndiffqty = $item['iqtyonhand'] - ($soldqty['soldqty'] ?? 0) - ($item['ndebitqty'] * $itemdetail['npack']);
