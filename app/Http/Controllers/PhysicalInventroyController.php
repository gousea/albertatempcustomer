...
class PhysicalInventroyController extends Controller
{
    ...
    public function get_scanned_data(){
	    ...
	    $select_query = "SELECT mi.iitemid, mi.npack, mi.vbarcode, mi.iqtyonhand, mi.vitemname, mi.vitemcode, (mi.new_costprice/mi.nsellunit) as unitcost FROM mst_item as mi WHERE mi.iitemid IN ('".$ids."') ORDER BY mi.iitemid ";
        ...
        if (isset($input['conditions']) && !empty($input['conditions']) && $input['conditions'] == 'all') {
            ...
            if (isset($input['price_select_by']) && !empty(trim($input['price_select_by'])) && isset($input['select_by_value_1']) && !empty($input['select_by_value_1'])) {
                ...
                if ($price_select_by == 'greater') {
                    ...
                    $select_by_value_2 = (isset($input['select_by_value_2'])? $input['select_by_value_2']:'');
    ...
    public function show(Request $request)
    {
        ...
        if (isset($input['ipiid'])){
            ...
            $sql = "SELECT SUM(tpid.ndebitqty) as total_counted, SUM(tpid.ndebitqty + tpid.ndiffqty) as total_expected, SUM(tpid.ndiffqty) as total_difference,
                    SUM(tpid.ndiffqty * (mi.new_costprice/mi.nsellunit)) as total_difference_cost,
                    SUM(tpid.ndiffqty * mi.dunitprice) as total_difference_price,
                    SUM(tpid.ndebitqty * (mi.new_costprice/mi.nsellunit)) as total_counted_cost,
                    SUM(tpid.ndebitqty * mi.dunitprice) as total_counted_price,
                    SUM((tpid.ndebitqty + tpid.ndiffqty) * (mi.new_costprice/mi.nsellunit)) as total_expected_cost,
                    SUM((tpid.ndebitqty + tpid.ndiffqty) * mi.dunitprice) as total_expected_price,
                    SUM((tpid.ndebitqty - (tpid.ndebitqty + tpid.ndiffqty)) * (mi.dunitprice - (mi.new_costprice/mi.nsellunit))) as total_profit_loss_difference
    ...
    public function getshow_data(Request $request)
    {
        ...
        foreach($items as $k => $item){
            ...
            $cost_price_vitemcode = DB::connection('mysql_dynamic')->select("SELECT (new_costprice/nsellunit) as unitcost, dunitprice, vitemcode FROM mst_item WHERE iitemid = '".$item['vitemid']."' ");
        ...
        if ($request->isMethod('post')) {
			...
			if(isset($input['ipiid']) && $input['ipiid'] != ''){
                ...
                foreach ($items as $k => $item) 
                {
                    ...
                    if(empty($soldqty['soldqty'])){
                        $soldqty['soldqty'] = 0;
                    }
                    
                    $ndiffqty = (int)($item['iqtyonhand'] ) - ($soldqty['soldqty']) - (int)($item['ndebitqty']);
                    
                    
                    //=====creating values for bulk insert===================
					$sql[] = "( '" . (int)$ipiid . "', '" . ($itemdetail['iitemid']) . "', \"" . (str_replace('"', '', ($itemdetail['vitemname']))) . "\", ' ', ' ', '" . ($item['ndebitqty']) . "', '0.00', '0.00', '0.00', '0.00', '" . ($ndebitextprice) . "', '0.00', '0.00', '0.00', '" . ($itemdetail['vbarcode']) . "', ' ', '" . ($ndiffqty) . "', ' ', '" . ($itemdetail['npack']) . "', '" . ($itemdetail['nunitcost']) . "', '" . ($itotalunit) . "', '" . (int)(session()->get('sid'))."', '". $itemdetail['iqtyonhand']."', '".$item['ndebitqty'] ."' )";
			...
			if(isset($input['ipiid']) && $input['ipiid'] != ''){
                ...
                if(isset($results) && $results != null){
                    foreach ($results as $k => $item) {
                        ...
                        if(empty((int)$item['ndebitqty'])){
                            $item['ndebitqty']=0;
                        }
                        
                        $itotalunit = (int)$item['ndebitqty'];
                        ...
                        $sql_ndebitqty .= "WHEN ".$item['iitemid']." THEN ".(int)$item['ndebitqty']." ";
                        ...
                        $sql_afterQOH  .= "WHEN ".$item['iitemid']." THEN ".(int)$item['ndebitqty']." ";
                ...
                if ($handle) {
                    ...
                    while (($strline = fgets($handle)) !== false) {
                        ...
                        if($line_row_index > 1){
                            ...
                            $itemdetail_query = "SELECT iitemid, vitemcode, (new_costprice/nsellunit) as unitcost, dunitprice FROM mst_item WHERE vbarcode  = '".strval($values[0])."' ";
    ...
    public function display_selected_data(Request $request)
    {
        ...
        if($check['count'] >= 1){
            ...
            if($check_status['estatus'] == 'Open'){
                ...
                mi.vitemcode, mi.dunitprice, (mi.new_costprice/mi.nsellunit) as unitcost, tpd.ndebitqty FROM 
                ...
                if(count($itemdata) > 0){
                    foreach ($itemdata as $key => $value) {
                        ...
                        $itemdata_query = "SELECT mpis.npack, mpis.iqtyonhand, mi.vitemcode, mi.dunitprice, (mi.new_costprice/mi.nsellunit) as unitcost FROM mst_physical_inventory_snapshot AS mpis LEFT JOIN mst_item as mi ON(mpis.iitemid = mi.iitemid) WHERE mpis.ipiid = '".$ipiid."' AND mpis.iitemid = '".$value['iitemid']."' ";
            ...
            $select_query = "SELECT mpis.iitemid, mpis.npack, mpis.vbarcode, mpis.iqtyonhand, mi.vitemname, mi.vitemcode, mi.dunitprice, (mi.new_costprice/mi.nsellunit) as unitcost FROM mst_physical_inventory_snapshot as mpis LEFT JOIN mst_item as mi ON(mpis.iitemid = mi.iitemid) WHERE mpis.ipiid = '".$ipiid."' ORDER BY mpis.vbarcode LIMIT ". $input['start'].", ".$limit;
            $itemdata = DB::connection('mysql_dynamic')->select($select_query);
    ...
    public function export_csv(Request $request)
    {
        ...
        if(isset($input['action']) && $input['action'] == 'edit_calculate'){
            ...
            foreach($result as $row){
                ...
                $row_data = array("`".$row['vbarcode']."`", $row['vitemname'], $row['iqtyonhand'], $row['ndebitqty']);
                $row_data = implode(",", $row_data);
            ...
            foreach($result as $row){
                // array_push($row,"0");
                // $row_data = implode(",", $row);
                $row_data = array("`".$row['vbarcode']."`", $row['vitemname'], $row['iqtyonhand'], 0);
                $row_data = implode(",", $row_data);
                $row_data = $row_data.PHP_EOL;
                // fputcsv($output, $row);
                fwrite($output, $row_data);
			...
			if(isset($input['import_physical_inventory_file']) && $request->hasFile('import_physical_inventory_file')){
                ...
                //   dd($query);
                    ...
                    $conn->query($query);
                     ...
                     //===Remove "`"(tilt sign) from vbarcode column which is added at the time of export
                    DB::connection('mysql_dynamic')->statement("update import_physical_inventory_csv_table set vbarcode = replace(vbarcode, '`', '')");
    ...
    public function import_display(Request $request)
    {
        ...
        if($check['count'] >= 1){
            ...
            // $select_query = "SELECT mpis.vbarcode,mi.vitemname, mi.vitemcode, mi.dunitprice, mpis.npack, (mi.new_costprice/mi.nsellunit) as unitcost, 
