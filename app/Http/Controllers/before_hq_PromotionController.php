    ...
    public function savepromotion(Request $req){
       ...
       //echo $prom_id." inserted successfully";
        ...
        if(!empty($input['added_promotion_items_text']) && strlen($prom_id)>0)
        {
            ...
            foreach($buyItems as $key => $value)
            {
                ...
                $itemDetails=$this->getItem($value);
               ...
               //dd($itemDetails);
                ...
                if(!empty($itemDetails)>0)
                {
                    ...
                    //$discount_price_list[$value] ? $discount_price_list[$value] : 0;
                  ...
                  //  $disc_value = $value;
                    if($input['promotion_type'] == 10){
                        ...
                        \DB::connection('mysql_dynamic')->insert("INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', barcode ='".$itemDetails['vbarcode']."', unit_price ='".$itemDetails['dunitprice']."', 
                        discounted_price ='".$new_price."', 
                        SID ='".session()->get('sid')."' ");
                    } else {
                        
                        $insert_data = "INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', barcode ='".$itemDetails['vbarcode']."', 
                        unit_price ='".$itemDetails['dunitprice']."', discounted_price ='".$disc_value."',
                        SID ='".session()->get('sid')."' ";
     ...
     public function updatepromotion(Request $req){
       ...
       //time not updating currect 
        $update_query_time = "UPDATE trn_promotions SET LastUpdate =now()";
        $query =  DB::connection('mysql_dynamic')->select($update_query_time);
        ...
        if(!empty($input['added_promotion_items_text']) && $prom_id) {
            ...
            foreach($existing_items as $et){
                ...
                if(in_array($et['item_id'], $input['added_promotion_items_text'])){
                    
                    // add it in the items to be updated
                    // $existing['update'][] = $et['item_id'];
                    
                    $itemDetails = $this->getItem($et['item_id']);
                   ...
                   // return $itemDetails ;
        		    if(!empty($itemDetails)) {
                        ...
                        if($input['promotion_type'] == 10){
    		                ...
    		                $update_query = "UPDATE trn_prom_details SET `prom_id`='{$prom_id}', `barcode`='{$itemDetails['vbarcode']}',
    		                `unit_price`='{$itemDetails['dunitprice']}', `discounted_price`='{$new_price}', LastUpdate =now(),
    		                SID ='{$itemDetails['SID']}' WHERE `prom_detail_id`={$et['prom_detail_id']}";

    		            } else {
    		                $update_query = "UPDATE trn_prom_details SET `prom_id`='{$prom_id}', `barcode`='{$itemDetails['vbarcode']}',
    		                `unit_price`='{$itemDetails['dunitprice']}', `discounted_price`='{$disc_value}', LastUpdate =now(),
    		                 SID ='{$itemDetails['SID']}' WHERE `prom_detail_id`={$et['prom_detail_id']}";
    		            }
                        //  dd($update_query);   		            
    		          //  $this->db2->query($update_query);
    		           $query =  DB::connection('mysql_dynamic')->select($update_query);
                    ...
                    // $this->db2->query($delete_query);
                      $query =  DB::connection('mysql_dynamic')->select($delete_query);
                    
                    
                    $insert_delete_query = "INSERT INTO `mst_delete_table` (`TableName`, `TableId`, `Action`,  `SID`) VALUES('trn_prom_details', '". (int)$et['prom_detail_id'] . "', 'delete',  '".session()->get('sid')."')";
                    // $this->db2->query($insert_delete_query);
                     $query =  DB::connection('mysql_dynamic')->select($insert_delete_query);
                }
                
                // Remove the items from the input that have already been dealt with.
                $remove_key = array_search($et['item_id'], $input['added_promotion_items_text']);
                if($remove_key !== FALSE){
            ...
            foreach($input['added_promotion_items_text'] as $prom_item){
                ...
                $itemDetails = $this->getItem($prom_item);
		        
		        if(!empty($itemDetails)) {
		            
		            $disc_value = isset($discount_price_list[$prom_item]) ? $discount_price_list[$prom_item] : 0;
		            ...
		            if($input['promotion_type'] == 10){
		                ...
		                $query_insert = "INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', barcode ='".$itemDetails['vbarcode']."', unit_price ='".$itemDetails['dunitprice']."', discounted_price ='".$new_price."',  SID ='".$itemDetails['SID']."' ";
		            } else {
		                $query_insert = "INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', barcode ='".$itemDetails['vbarcode']."', unit_price ='".$itemDetails['dunitprice']."', discounted_price ='".$disc_value."',  SID ='".$itemDetails['SID']."' ";
		            }
		            
                  $query =  DB::connection('mysql_dynamic')->select($query_insert);
...
// 		if(!empty($input['promotion_customers']) && $prom_id) {
// 		    $this->db2->query("DELETE FROM  trn_prom_customers WHERE prom_id = '" . (int)$prom_id . "'");
		    
// 		    $customers = $input['promotion_customers'];
       ...
       // $this->load->model('administration/promotion');
        if (isset($input['buyItems'])  && !empty($input['buyItems'])) {
            ...
            foreach($items as $value){
                ...
                {
                    $tableRows .= "<tr>";
                    $tableRows .= "<td><input type='checkbox' class='promotion_buy_items' name='added_promotion_items[]' value=".$itemId."><input type='hidden' class='' name='added_promotion_items_text[]' value=".$itemId."></td>";
                ...
                elseif($prom_type == 10)
                {
                    ...
                    $addlDiscount = $addl_discount == 0 ? $slab_price : $addl_discount;
        ...
        if (isset($input['buyItems'])  && !empty($input['buyItems'])) {
            ...
            foreach($items as $value){
                ...
                elseif($prom_type == 10)
                {
                    ...
                    $addlDiscount = number_format($addl_discount/$buy_qty , 2);
...
//      }
    ...
    //     $sql = "INSERT INTO trn_promotions 
        ...
        \DB::connection('mysql_dynamic')->select($sql);
            ...
            foreach($buyItems as $key => $value)
            {
                ...
                if(!empty($itemDetails))
                {
                    ...
                    if($data['promotion_type'] == 10){
                        ...
                        $new_price = number_format($new_price , 2);
                        \DB::connection('mysql_dynamic')->select("INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', barcode ='".$itemDetails['vbarcode']."', unit_price ='".$itemDetails['dunitprice']."', discounted_price ='".$new_price."', 
                        SID ='".$itemDetails['SID']."' ");
                    } else {
                        
                        \DB::connection('mysql_dynamic')->select("INSERT INTO trn_prom_details SET prom_id ='".$prom_id."', 
    ...
    public function deletePromotions(Request $req) {
        ...
        foreach ($input as $del) {
            ...
            //complite delete promotion   every item PK(prom_detail_id) inserting in mst_delete_table
    ...
    function getSavedBuyItems($prom_id) {
        $query = \DB::connection('mysql_dynamic')->select("SELECT mi.iitemid,mi.vitemcode,REPLACE(mi.vitemname,'''','') as vitemname,
