...
class BuyDownController extends Controller
{
    ...
    public function getSavedItemsAjax(Request $request)
    {
        ...
        foreach($results1 as $results ){
                ...
                $row .= "<td class='price'>".number_format((float)$results->price_before_buydown, 2, '.', '')." <input type='hidden' name='added_buydown_before_price[]' value=".$results->price_before_buydown."></td>";
                $row .= "<td><input type='text' class='buydown_amt' id='buydown_row' name='added_buydown_amt[]' value=".$results->buydown_amount." ></td>";
                $row .= "<td class='cost_after_buydown'>". number_format((float)($results->cost_after_buydown), 2, '.', '') ." </td>";
                $row .= "<td class='price_after_buydown'>". number_format((float)($results->price_after_buydown), 2, '.', '') ."</td>";
                $row .= "</tr>";
        }
        echo $row;
    }
    ...
    public function update(Request $request)
    {
        ...
        foreach($result as $key=>$val){
            $query = DB::connection('mysql_dynamic')->select("DELETE FROM trn_buydown_details WHERE buydown_id = '".$buydown_id."' AND vbarcode ='".$val."'" );
        }
