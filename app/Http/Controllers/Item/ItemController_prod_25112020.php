    ...
    public function checkVendorExists(Request $request){
        ...
        if($vsuppliercode){
            ...
            foreach($stores as $store){
                $dup_query = "Select vcompanyname from u".$store.".mst_supplier where  vcompanyname = '".$vcompanyNmae[0]->vcompanyname."' ";
                $dup_entry = DB::connection("mysql")->select($dup_query);
                if(count($dup_entry) == 0){
                    array_push($notExistStore, $store); 
                }
            }
        }
    ...
    public function action_vendor(Request $request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if($new_database === false){
                ...
                $olditem = new Olditem();
                $result = $olditem->addUpdateItemVendor($temp_arr);
                ...
                $item = new Item();
                $result = $item->addUpdateItemVendor($temp_arr);
            ...
            if($new_database === false){
				...
				if(isset($input['itemvendors']) && count($input['itemvendors']) > 0){
					foreach ($input['itemvendors'] as $key => $value) {
						$olditem = new Olditem();
    			        $result = $olditem->addUpdateItemVendor($value);
					}
				}
                ...
                if(isset($input['itemvendors']) && count($input['itemvendors']) > 0){
                    foreach ($input['itemvendors'] as $key => $value) {
                        $item = new Item();
                        $result = $item->addUpdateItemVendor($value);
                    }
                }
                if(isset($result['success'])){
                session()->put('success', $result['success']);
