...
class StoreDatabaseSelection
{
    ...
    public function handle($request, Closure $next )
    {
        ...
        if(Session::has('dbhost') && Session::has('dbname') && Session::has('dbuser') && Session::has('dbpassword')){
            ...
            if(strpos($request->path(), "item_audit_list") !== false){
                $name = "RouteName: ".$request->path().PHP_EOL;
                fwrite($myfile, $name);
                $name = "Store Id: ".session()->get('sid').PHP_EOL;
                fwrite($myfile, $name);
            } 
           ...
           if(strpos($request->path(), "item_audit_list") !== false){
            ...
            if(isset($data[0]) && count((array)$data[0]) > 0){
                            ...
                            if(Auth::user()->user_role == "StoreAdmin" && Auth::user()->sid == 0){
                                ...
                                if(count($userPermsData) <= 0){
                                    ...
                                    foreach ($permissions as $permission) {
                                        ...
                                        $insert = DB::connection('mysql_dynamic')->statement($user_perms_insert);
                                    } 
                                    $userPermsData = DB::connection('mysql_dynamic')->select("SELECT  distinct(mp.vpermissioncode) FROM mst_permission mp join mst_userpermissions mup on mp.vpermissioncode = mup.permission_id where mp.vpermissiontype = 'WEB' and mup.status = 'Active' and mup.userid = '".$user_id."' ");
                                }else {
                                    $userPermsData = DB::connection('mysql_dynamic')->select("SELECT  distinct(mp.vpermissioncode) FROM mst_permission mp join mst_userpermissions mup on mp.vpermissioncode = mup.permission_id where mp.vpermissiontype = 'WEB' and mup.status = 'Active' and mup.userid = '".$user_id."' ");
                ...
                //get the session valvue of head quater
                ...
                //======endof the versions code 
            
                // get the list of child stores
                if($data[0]->hq_sid == 1){
    ...
    public function DatabaseAction($store_id)
    {
        ...
        $sql_excute = DB::connection('mysql_dynamic')->statement($sql);
        $sql_user_permissions_excute = DB::connection('mysql_dynamic')->statement($sql_user_permissions);
        ...
        $insert_data_excute = DB::connection('mysql_dynamic')->statement($insert_data);
        
        $permissions = DB::connection('mysql_dynamic')->statement("SELECT  distinct(vpermissioncode) FROM mst_permission where vpermissiontype = 'WEB' ");
